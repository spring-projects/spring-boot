...
public class RedisAutoConfigurationTests {
	...
	public void testOverrideRedisConfiguration() throws Exception {
		...
		assertThat(this.context.getBean(JedisConnectionFactory.class).getHostName()).isEqualTo("foo");
	...
	public void testOverrideUrlRedisConfiguration() throws Exception {
		load("spring.redis.host:foo", "spring.redis.password:xyz", "spring.redis.port:1000", "spring.redis.ssl:true",
				"spring.redis.url:redis://user:password@example:33");
		assertThat(this.context.getBean(JedisConnectionFactory.class).getHostName()).isEqualTo("example");
		assertThat(this.context.getBean(JedisConnectionFactory.class).getPort()).isEqualTo(33);
		assertThat(this.context.getBean(JedisConnectionFactory.class).getPassword()).isEqualTo("password");
		assertThat(this.context.getBean(JedisConnectionFactory.class).isUseSsl()).isEqualTo(true);
	}
	...
	public void testPasswordInUrlWithColon() {
		...
		assertThat(this.context.getBean(JedisConnectionFactory.class).getHostName()).isEqualTo("example");
		assertThat(this.context.getBean(JedisConnectionFactory.class).getPort()).isEqualTo(33);
		assertThat(this.context.getBean(JedisConnectionFactory.class).getPassword()).isEqualTo("pass:word");
	}
	...
	public void testRedisConfigurationWithPool() throws Exception {
		...
		assertThat(this.context.getBean(JedisConnectionFactory.class).getHostName()).isEqualTo("foo");
		assertThat(this.context.getBean(JedisConnectionFactory.class).getPoolConfig().getMaxIdle()).isEqualTo(1);
	}
	...
	public void testRedisConfigurationWithTimeout() throws Exception {
		...
		assertThat(this.context.getBean(JedisConnectionFactory.class).getHostName()).isEqualTo("foo");
		assertThat(this.context.getBean(JedisConnectionFactory.class).getTimeout()).isEqualTo(100);
	}
	...
	public void testRedisConfigurationWithSentinel() throws Exception {
		...
		if (isAtLeastOneNodeAvailable(sentinels)) {
			load("spring.redis.sentinel.master:mymaster",
					"spring.redis.sentinel.nodes:" + StringUtils.collectionToCommaDelimitedString(sentinels));
			assertThat(this.context.getBean(JedisConnectionFactory.class).isRedisSentinelAware()).isTrue();
		}
	}
	...
	public void testRedisConfigurationWithCluster() throws Exception {
		...
		if (isAtLeastOneNodeAvailable(clusterNodes)) {
			...
			assertThat(this.context.getBean(JedisConnectionFactory.class).getClusterConnection()).isNotNull();
		}
	}
	...
	private AnnotationConfigApplicationContext doLoad(String... environment) {
		...
		applicationContext.register(RedisAutoConfiguration.class, PropertyPlaceholderAutoConfiguration.class);
