...
public abstract class TestResultsOverview
	...
	/**
	 * This method is called when the build finishes. It implements the
	 * OperationCompletionListener interface to defer the close operation until the build
	 * ends.
	 ...
	 *
	 * This method is responsible for printing any test failures found during the
	 * execution of test tasks. If no test failures are found, the method returns without
	 * performing any action.
	 *
	 * The test failures are printed to the standard error stream in a formatted manner.
	 * The number of test failures found is displayed along with the corresponding test
	 * task paths. For each test task, the class name and name of each failure are
	 * printed.
	 *
	...
	private static final class TestFailure implements Comparable<TestFailure> {
		...
		/**
		 * Compares this TestFailure object with the specified TestFailure object for
		 * order. The comparison is based on the class name and then the method name of
		 * the TestFailure objects.
		 * @param other the TestFailure object to be compared
		 * @return a negative integer, zero, or a positive integer as this TestFailure
		 * object is less than, equal to, or greater than the specified TestFailure object
		 */
		@Override
