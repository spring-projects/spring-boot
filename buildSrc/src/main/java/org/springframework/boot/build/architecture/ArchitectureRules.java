...
final class ArchitectureRules {
	...
	private static ArchCondition<JavaMethod> haveNoParameters() {
		return check("have no parameters", ArchitectureRules::haveNoParameters);
	}

	private static void haveNoParameters(JavaMethod item, ConditionEvents events) {
		List<JavaParameter> parameters = item.getParameters();
		if (!parameters.isEmpty()) {
			events.add(SimpleConditionEvent.violated(item, item.getDescription() + " should have no parameters"));
		}
	}
