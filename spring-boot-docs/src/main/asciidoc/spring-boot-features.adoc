...
If `'spring-integration-http'` is preset on the classpath, the `@EnableIntegrationGraphController` will be switched on with `/integration` as default request mapping path.
That path can be overridden with `spring.integration.graph-controller-path` configuration property.

In addition all Spring Integration global properties is exposed as an `IntegrationProperties` `@ConfigurationProperties`.
