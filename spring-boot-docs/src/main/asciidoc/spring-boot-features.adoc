...
[boot-features-testing-spring-boot-applications-testing-autoconfigured-elasticsearch-test]]
`@DataElasticSearchTest` can be used if you want to test ElasticSearch applications. By default, it will
configure a local ElasticSearch (if available), configure an `ElasticSearchTemplate`, scan
for `@Document` classes and configure Spring Data ElasticSearch repositories. Regular
`@Component` beans will not be loaded into the `ApplicationContext`:
...
----
	...
	public class ExampleDataElasticSearchTests {
...
A list of the auto-configuration that is enabled by `@DataElasticSearchTest` can be
<<appendix-test-auto-configuration#test-auto-configuration,found in the appendix>>.
