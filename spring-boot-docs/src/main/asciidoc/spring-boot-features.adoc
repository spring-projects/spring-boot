...
=== GemFire
http://pivotal.io/big-data/pivotal-gemfire#details[Pivotal GemFire] is an In-Memory
Data Grid (IMDG), key-value store, with elastic compute and data management features,
capable of linear scale out in a shared nothing architecture for real-time,
low-latency/high-throughput data access.

https://github.com/spring-projects/spring-data-gemfire[Spring Data GemFire] uses the
powerful and consistent Spring programming model to provide a convenient and highly
familiar framework for accessing the
https://pivotal.io/big-data/pivotal-gemfire[Pivotal GemFire] data management platform.

Spring Boot provides a convenient '`Starter`' (`spring-boot-starter-data-gemfire`)
for collecting the necessary dependencies to easily build Spring-powered, Pivotal GemFire
applications.  In addition, there is also support for enabling Spring Data GemFire
...
[[boot-features-connecting-to-gemfire]]
==== Connecting to Pivotal GemFire using Spring Data GemFire

To use Spring Boot's auto-configuration support for Spring Data GemFire's Repository
infrastructure extension, not only do you need Spring Data GemFire and Pivotal GemFire
on your classpath, in addition to your application-defined Repositories, but you also
need to define a GemFire cache instance.  There are 2 options for defining a GemFire
cache depending on the GemFire topology choice used in your application's architecture.

First, you might choose to embed a GemFire cache in your application, participating as a
http://gemfire.docs.pivotal.io/docs-gemfire/latest/topologies_and_comm/p2p_configuration/chapter_overview.html[peer member]
in the GemFire cluster. To do so, you must define a peer cache instance using
Spring Data GemFire's `CacheFactoryBean`...
...
class PeerCacheConfiguration {
	...
	Properties gemfireProperties() {
		...
		gemfireProperties.setProperty("name", "PeerExample");
...
You might also choose to use GemFire's
http://gemfire.docs.pivotal.io/docs-gemfire/latest/topologies_and_comm/cs_configuration/chapter_overview.html[client/server topology]
to connect to a cluster of GemFire Servers running elsewhere.  If so, you must configure
your application as a GemFire cache client using Spring Data GemFire's
...
class ClientCacheConfiguration {
	...
	Properties gemfireProperties() {
		...
		gemfireProperties.setProperty("name", "ClientExample");
		gemfireProperties.setProperty("log-level", "config");
	...
	PoolFactoryBean gemfirePool() {
		...
		gemfirePool.setFreeConnectionTimeout(5000);
		gemfirePool.setIdleTimeout(30000);
		gemfirePool.setMinConnections(50);
		gemfirePool.setPrSingleHopEnabled(true);
		gemfirePool.setPingInterval(20000);
...
The GemFire `Pool` is a pool of connections needed by a cache client to communicate to
the GemFire servers in the cluster during Region data access operations.

[[boot-features-spring-data-gemfire-repositories]]
==== Spring Data GemFire Repositories

Regardless of the GemFire topology used, either 'peer-to-peer' (P2P) or 'client/server',
you need to define the Regions that will store your application data.

With a peer cache, you have a choice of different
http://gemfire.docs.pivotal.io/docs-gemfire/latest/developing/region_options/region_types.html[Region types]
corresponding to different data management policies and consistency options. You can
choose from `LOCAL`, `REPLICATE` and `PARTITION`.  On the client, a developer can choose
from one of several
http://data-docs-samples.cfapps.io/docs-gemfire/latest/javadocs/japi/com/gemstone/gemfire/cache/client/ClientRegionShortcut.html[ClientRegionShortcuts].

When using Spring Data GemFire, the developer only need define an appropriate SDG
...
`PartitionedRegionFactoryBean`.  As a client cache application, the developer just
...
Spring Data GemFire's `@Region` annotation...
...
Then, you define the corresponding Spring Data (GemFire) Repository...
...
your application Repositories with Spring Data GemFire.
...
along with the
http://gemfire.docs.pivotal.io/docs-gemfire/about_users_guide.html[Pivotal GemFire User's Guide].
