...
----
	...
	endpoints.configprops.keys-to-sanitize=password,secret,REGEX:.*password.* # suffix or regex
	...
	endpoints.env.keys-to-sanitize=password,secret,REGEX:.*password.* # suffix or regex
