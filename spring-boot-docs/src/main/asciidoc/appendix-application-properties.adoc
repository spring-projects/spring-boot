...
----
	...
	spring.rabbitmq.listener.container-type= # The listener container type `simple` or `direct`; default `simple`.
	...
	spring.rabbitmq.listener.simple.transaction-size= # Number of messages to be processed in a transaction; number of messages between acks for a `simple` container. For best results it should be less than or equal to the prefetch count.
