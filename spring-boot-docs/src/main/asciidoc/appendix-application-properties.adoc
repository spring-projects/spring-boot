...
----
	...
	spring.integration.task-scheduler-pool-size=10 # The number of threads available in the default taskScheduler.
	spring.integration.throw-exception-on-late-reply=false # When `true`, messages that arrive at a gateway reply channel will throw an exception.
	spring.integration.component-annotation-required=false # When `true`, Messaging Annotation Support requires a declaration of the `@MessageEndpoint`.
	spring.integration.graph-controller-path=/integration # Request mapping path that serves as the base URI for the Integration Graph Controller.
	spring.integration.channels.auto-create=true # When true, `input-channel` s will be automatically declared as `DirectChannel` s when not explicitly found in the application context.
	spring.integration.channels.max-unicast-subscribers=Integer.MAX_VALUE # This property provides the default number of subscribers allowed on, say, a `DirectChannel`.
	spring.integration.channels.max-broadcast-subscribers=Integer.MAX_VALUE # This property provides the default number of subscribers allowed on, say, a `PublishSubscribeChannel`.
