...
As described in the http://docs.aws.amazon.com/elasticbeanstalk/latest/dg/create_deploy_Java.html[official]
Elastic Beanstalk java guide, there are two main options to deploy a Java application.
...
Elastic Beanstalk environments run an nginx instance on port 80 to proxy the actual
application, running on port 5000. To configure it add the below to *application.properties*
----
server.port=5000
----
...
By default Elastic Beanstalk uploads sources and compile them in AWS. To upload the binaries instead,
add the below to the *.elasticbeanstalk/config.yml* file
...
to avoid it, set the environment type to "Single instance" as decribed http://docs.aws.amazon.com/elasticbeanstalk/latest/dg/environments-create-wizard.html#environments-create-wizard-capacity[here].
Single instance environments can be created using the CLI as well using the below command:
----
eb create -s
----
