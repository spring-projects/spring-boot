...
As a security best practice login should be disabled for the user account. This can be
achieved by specifying a shell such as `/sbin/nologin` for the user account.
...
|`SU_OPTS`
|Additional options for the `su` command when starting the Spring Boot app 
 (`-s /bin/sh` by default). On BSD you should change this value to `-m` if you want to 
 support `/sbin/nologin` as the default user shell.
