...
Since version 5, Hibernate defines `Physical` and `Implicit` naming strategies which you
can configure in Spring Boot by providing a fully qualified class name to the properties
`spring.jpa.hibernate.naming.physical-strategy` and
`spring.jpa.hibernate.naming.implicit-strategy`. Spring Boot configures
