...
NOTE: The `spring-boot:run` maven target adds src/main/resources directly to 
the classpath (for hot reloading purposes). This circumvents the resource 
filtering and this feature. You can use the `exec:java` target or configure 
addResources to false for the `spring-boot:run` target (disabling hot 
reloading), see the 
{spring-boot-maven-plugin-site}/usage.html[plugin usage page].
