...
| `env` +
(`spring-boot.build-image.env.<ENVVAR_NAME>`)
...
Environment variables can also be specified on the command line using system properties with the prefix `spring-boot.build-image.env.`, as shown in the following example:

[source,shell]
----
$ mvn spring-boot:build-image \
      -Dspring-boot.build-image.env.BP_JVM_VERSION=17 \
      -Dspring-boot.build-image.env.HTTPS_PROXY=https://proxy.example.com
----

System properties override environment variables specified in the plugin configuration.
