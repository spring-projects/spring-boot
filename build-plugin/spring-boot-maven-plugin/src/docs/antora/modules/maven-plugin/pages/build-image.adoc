...
Applications that require a shell, or that depend upon a system library that is not present, should override the `runImage` parameter to use one that includes a shell and a broader set of system libraries, such as `paketobuildpacks/ubuntu-resolute-run:latest`.
