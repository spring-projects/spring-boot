...
13. Configures any `JavaCompile` tasks that use `spring-boot-configuration-processor` to preserve property descriptions across incremental compilations by caching the previously generated metadata in the task's temporary directory.
...
Additionally, when the Protobuf `grpc` plugin is configured, the Spring Boot plugin:

. Configures it to use the artifact `io.grpc:protoc-gen-grpc-java`, aligning its version with that of gRPC dependencies on the runtime classpath.
