...
| `paketobuildpacks/ubuntu-resolute-run-tiny:latest` if `builder` is not set, otherwise the run image specified in Builder metadata.
...
NOTE: When `builder` is not set, the run image defaults to `paketobuildpacks/ubuntu-resolute-run-tiny:latest`, which contains a reduced set of system libraries and does not include a shell.
If `builder` is set, the run image from the builder metadata is used.
