...
NOTE: With the default builder `paketobuildpacks/ubuntu-resolute-builder:latest`, the run image defaults to `paketobuildpacks/ubuntu-resolute-run-tiny:latest`, which contains a reduced set of system libraries and does not include a shell.
