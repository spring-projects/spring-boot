...
public class SpringApplicationBuilderTests {
	...
	public void parentFirstCreationWithProfileAndDefaultArgs() throws Exception {
		SpringApplicationBuilder application = new SpringApplicationBuilder(ExampleConfig.class).profiles("node")
				.properties("transport=redis").child(ChildConfig.class).web(false);
		this.context = application.run();
		...
		assertThat(this.context.getParent().getEnvironment().acceptsProfiles("node")).isTrue();
	...
	public void parentFirstWithDifferentProfile() throws Exception {
		SpringApplicationBuilder application = new SpringApplicationBuilder(ExampleConfig.class).profiles("node")
				.properties("transport=redis").child(ChildConfig.class).profiles("admin").web(false);
		this.context = application.run();
		assertThat(this.context.getEnvironment().acceptsProfiles("node", "admin")).isTrue();
		assertThat(this.context.getParent().getEnvironment().acceptsProfiles("admin")).isFalse();
	}
	...
	public void parentWithDifferentProfile() throws Exception {
		...
		this.context = application.run();
		assertThat(this.context.getEnvironment().acceptsProfiles("node", "admin")).isTrue();
		assertThat(this.context.getParent().getEnvironment().acceptsProfiles("node", "parent")).isTrue();
		assertThat(this.context.getParent().getEnvironment().acceptsProfiles("admin")).isFalse();
	}
