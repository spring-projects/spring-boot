...
* The build includes checkstyle rules for many of our code conventions. Run
  `./mvnw validate` if you want to check your changes are compliant in Maven.
  or `./gradlew checkstyleMain checkstyleTest` if you want to check your changes are
  compliant in gradle.
...
https://eclipse.org/m2e/[M2Eclipse] eclipse plugin for maven support and
...
##Maven
https://maven.apache.org/run-maven/index.html[Apache Maven] on (OpenJDK, ...) >= 11 or Oracle JDK >= 1.8 and 
Maven >= 3, this allows it to be used with QuickPerf also.
We include '`Maven Wrapper`' scripts (`./mvnw` or `mvnw.bat`) that you can run rather than
