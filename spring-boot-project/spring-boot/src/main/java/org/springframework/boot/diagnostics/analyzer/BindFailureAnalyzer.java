...
class BindFailureAnalyzer extends AbstractFailureAnalyzer<BindException> {
	...
	/**
	 * Analyzes the given BindException and its root cause to determine the failure
	 * analysis.
	 * @param rootFailure the root failure that occurred
	 * @param cause the BindException that caused the failure
	 * @return the FailureAnalysis object representing the analysis result, or null if the
	 ...
	 * @return a String containing the exception type and message in the format
	 ...
	 * @param missingParametersAnalysis the failure analysis for missing parameters, if
	 * any
