...
public class DeferredLog implements Log {
	 ...
	 * @return {@code true} if the fatal level logging is enabled, {@code false}
	 * otherwise.
	 */
	@Override
	 ...
	 * <p>
	 * This method is used to switch the destination of the DeferredLog by obtaining a new
	 * destination from the destinationSupplier. The destination is updated in a
	...
	static class Lines implements Iterable<Line> {
		...
		/**
		 * Adds a new line to the lines list with the specified destination supplier, log
		 * level, message, and throwable.
		 * @param destinationSupplier the supplier that provides the destination for the
		 * log
	...
	static class Line {
		...
		/**
		 * Constructs a new Line object with the specified destination supplier, log
		 * level, message, and throwable.
