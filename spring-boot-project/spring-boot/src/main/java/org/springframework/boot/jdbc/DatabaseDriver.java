...
public enum DatabaseDriver {
	...
	SQLSERVER("Microsoft SQL Server", "com.microsoft.sqlserver.jdbc.SQLServerDriver",
		...
		/**
		 * Checks if the given product name matches the product name of the database
		 * driver.
		 * @param productName the product name to be checked
		 * @return true if the product name matches the database driver's product name,
		 * false otherwise
		 */
		@Override
	...
	FIREBIRD("Firebird", "org.firebirdsql.jdbc.FBDriver", "org.firebirdsql.ds.FBXADataSource",
		...
		/**
		 * Returns a collection of URL prefixes for the Firebird database driver. The
		 * prefixes include "firebirdsql" and "firebird".
		 ...
		 * @param productName the product name to be checked
		 * @return true if the product name matches the database driver name, false
		 * otherwise
		 */
		@Override
		protected boolean matchProductName(String productName) {
	...
	DB2("DB2", "com.ibm.db2.jcc.DB2Driver", "com.ibm.db2.jcc.DB2XADataSource", "SELECT 1 FROM SYSIBM.SYSDUMMY1") {
		...
		/**
		 * Checks if the given product name matches the product name of the
		 * DatabaseDriver.
		 * @param productName the product name to be checked
	...
	DB2_AS400("DB2 UDB for AS/400", "com.ibm.as400.access.AS400JDBCDriver",
		...
		/**
		 * Checks if the given product name matches the name of the AS/400 database
		 * driver.
		 * @param productName the product name to be checked
		 * @return true if the product name matches the AS/400 database driver, false
		 * otherwise
		 */
		@Override
		protected boolean matchProductName(String productName) {
	...
	INFORMIX("Informix Dynamic Server", "com.informix.jdbc.IfxDriver", null, "select count(*) from systables") {
		...
		/**
		 * Returns a collection of URL prefixes for the Informix database driver. The URL
		 * prefixes include "informix-sqli" and "informix-direct".
		 * @return a collection of URL prefixes
		 */
		@Override
		protected Collection<String> getUrlPrefixes() {
	...
	/**
	 * Constructs a new DatabaseDriver with the specified product name and driver class
	 * name.
	...
	/**
	 * Constructs a new DatabaseDriver with the specified product name, driver class name,
	 * and XA data source class name.
	 * @param productName the name of the database product
	 ...
	 * @param productName the product name to be checked
	 * @return true if the given product name matches the stored product name, false
	 * otherwise
	 */
	protected boolean matchProductName(String productName) {
