	...
	private static final class LazyInitializationBeanFactoryPostProcessor
		...
		public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory)
			...
			for (String name : beanFactory.getBeanDefinitionNames()) {
				Optional<Lazy> initStrategy = Optional
						.ofNullable(beanFactory.findAnnotationOnBean(name, Lazy.class));
				if (isEagerEnforced(initStrategy))
					continue;
				beanFactory.getBeanDefinition(name).setLazyInit(true);
			}
		}

		private boolean isEagerEnforced(Optional<Lazy> initStrategy) {
			return initStrategy.isPresent() && !initStrategy.get().value();
		}
