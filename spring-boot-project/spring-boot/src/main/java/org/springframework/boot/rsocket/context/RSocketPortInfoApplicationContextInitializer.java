...
public class RSocketPortInfoApplicationContextInitializer
	...
	private static class Listener implements ApplicationListener<RSocketServerInitializedEvent> {
		...
		/**
		 * This method is called when the RSocket server is initialized. It sets the port
		 * property in the application context if the server address is not null.
		 * @param event The RSocketServerInitializedEvent object representing the server
