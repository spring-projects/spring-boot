...
class EphemeralBuilder {
	...
	/**
	 * Returns a Layer object representing the environment variables specified in the
	 * given map. The Layer object is created by iterating over the entries in the map and
	 * creating a file for each entry in the "/platform/env/" directory of the layer
	 * layout. The name of each file is constructed by appending the key of the entry to
	 * the "/platform/env/" directory path. The content of each file is set to the value
	 * of the entry, or an empty string if the value is null.
