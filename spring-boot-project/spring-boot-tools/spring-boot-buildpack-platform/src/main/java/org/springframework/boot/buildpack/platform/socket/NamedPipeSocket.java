...
public class NamedPipeSocket extends Socket {
	...
	/**
	 * Closes the NamedPipeSocket and releases any system resources associated with it. If
	 * the NamedPipeSocket's channel is not null, it will be closed.
	 * @throws IOException if an I/O error occurs while closing the NamedPipeSocket or its
	 * channel
	 */
	@Override
	...
	private static class AsynchronousFileByteChannel implements AsynchronousByteChannel {
		...
		/**
		 * Constructs a new AsynchronousFileByteChannel object with the specified
		 * AsynchronousFileChannel.
		 * @param fileChannel the AsynchronousFileChannel to be associated with the
		...
		/**
		 * Reads a sequence of bytes from the file into the given buffer, and invokes the
		 * specified completion handler when the operation completes.
		 ...
		 * @throws ReadPendingException if a read operation is already in progress on this
		 * channel
		 ...
		 * the current position. The operation is asynchronous and will be completed by
		 * invoking the given completion handler.
		...
		/**
		 * Closes the AsynchronousFileByteChannel and releases any system resources
		 * associated with it.
		...
		/**
		 * Returns a boolean value indicating whether the file channel associated with
		 * this asynchronous file byte channel is open.
		...
		private static final class CompletableFutureHandler extends CompletableFuture<Integer>
			...
			/**
			 * This method is called when the asynchronous read operation is completed. It
			 * takes in the number of bytes read and the attachment object as parameters.
			 * If the number of bytes read is greater than 0, it calls the complete method
			 * with the number of bytes read. Otherwise, it calls the complete method with
			 * -1.
			 * @param read The number of bytes read in the asynchronous read operation.
			 * @param attachment The attachment object associated with the asynchronous
			 * read operation.
			 */
			@Override
