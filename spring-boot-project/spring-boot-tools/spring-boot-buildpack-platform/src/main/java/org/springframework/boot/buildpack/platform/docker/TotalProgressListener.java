...
public abstract class TotalProgressListener<E extends ImageProgressUpdateEvent> implements UpdateListener<E> {
	...
	/**
	 * Called when the progress has reached its maximum value and is finished. This method
	 * iterates through all the layers and calls the finish method on each layer. It then
	 * publishes the progress value as 100.
	 */
	@Override
	...
	private static class Layer {
		...
		/**
		 * Initializes a new instance of the Layer class with the specified tracked status
		 * keys.
		 * @param trackedStatusKeys an array of strings representing the tracked status
		 * keys
		 */
		 ...
		 * @param event the ImageProgressUpdateEvent containing the progress update
		 * information
		 */
		 ...
		 * @param detail the progress detail object containing the current and total
		 * progress values
		...
		/**
		 * Sets the progress of all statuses to 100, indicating that the layer has been
		 * finished.
		 */
		void finish() {
