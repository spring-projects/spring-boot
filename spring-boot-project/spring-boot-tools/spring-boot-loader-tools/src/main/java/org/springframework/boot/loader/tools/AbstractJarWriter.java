...
public abstract class AbstractJarWriter implements LoaderClassesWriter {
	...
	/**
	 * Writes the entries of a JarFile to a destination, applying transformations and
	 * handling unpacking.
	...
	/**
	 * Sets up the given JarArchiveEntry by checking if it has a valid zip header and if
	 * its compression method is not stored. If the entry has a valid zip header and is
	 * not stored, it sets up the entry's compressed size and CRC. If the entry does not
	 * have a valid zip header or is stored, it sets the entry's compressed size to -1.
	 ...
	 * @return the timestamp of the first non-directory entry in the nested library, or
	 * the last modified timestamp of the library if an exception occurs
	 */
	 ...
	 * @return true if the entry is a directory entry and not the "META-INF/" directory,
	 * false otherwise
	 */
	 ...
	 * @return the updated EntryWriter with the unpack comment added, or the original
	 * EntryWriter if no unpacking is required
	 * @throws IOException if an I/O error occurs while writing the entry
	 */
	...
	private static class CrcAndSize {
		...
		/**
		 * Calculates the CRC (Cyclic Redundancy Check) and size of the given input
		 * stream.
