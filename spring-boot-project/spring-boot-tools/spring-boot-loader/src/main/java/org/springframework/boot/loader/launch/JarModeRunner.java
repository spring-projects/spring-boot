...
final class JarModeRunner {
	 ...
	 * It retrieves the jar mode from the system property "jarmode" and loads the
	 * available jar mode candidates using the SpringFactoriesLoader. It then iterates
	 * through the candidates and checks if any of them accepts the specified jar mode. If
	 * a candidate accepts the mode, it runs the candidate's run method passing the mode
	 * and command line arguments. If no candidate accepts the mode, an error message is
	 * printed to the standard error stream. If the system property "DISABLE_SYSTEM_EXIT"
	 * is not set to true, the application exits with a status code of 1.
