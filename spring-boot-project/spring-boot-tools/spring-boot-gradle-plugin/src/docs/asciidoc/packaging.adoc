...
The plugin adds a `META-INF/bundled-libraries.yaml` file to the packaged jar/war archive.
The file contains details about each library bundled within the archive.
Spring Boot Actuator reads this file at runtime to populate the `libraries` endpoint with details about the bundled libraries.
