...
The plugin's jar/war <<packaging-executable,packaging tasks>> generate this file based on the libraries bundled in the archive.

[[integrating-with-actuator-build-info]]
=== Generating Build Information
Spring Boot Actuator's `info` endpoint publishes information about your build if a `META-INF/build-info.properties` file is present.
