...
class JarFileEntries implements CentralDirectoryVisitor, Iterable<JarEntry> {
	...
	private final Map<Integer, FileHeader> entriesCache = Collections
		.synchronizedMap(new LinkedHashMap<>(16, 0.75f, true) {
			...
			/**
			 * Removes the eldest entry from the map if the size of the map exceeds the
			 * entry cache size.
	...
	/**
	 * Constructs a new JarFileEntries object with the specified JarFile and
	 * JarEntryFilter.
	...
	/**
	 * Sorts the entries in the JarFileEntries object and updates the positions array
	 * accordingly.
	 *
	 * This method uses the quicksort algorithm to sort the entries in ascending order
	 * based on their positions.
	 *
	 * After sorting, the positions array is updated to reflect the new positions of the
	 * entries.
	 * @param None
	 * @return None
	 */
	...
	/**
	 * Swaps the elements at the specified indices in the hashCodes,
	 * centralDirectoryOffsets, and positions arrays.
	 ...
	 * @return {@code true} if the JarFileEntries contains an entry with the specified
	 ...
	 * @return an InputStream for reading the contents of the entry, or null if the entry
	 * is null
	...
	/**
	 * Retrieves the entry with the specified hash code, name, suffix, and type from the
	 * JarFileEntries.
	 ...
	 * @return the entry with the specified hash code, name, suffix, and type, or null if
	 * not found
	 */
	...
	/**
	 * Applies the filter to the given name and returns the filtered name. If a filter is
	 * set, the filter will be applied to the name; otherwise, the original name will be
	 * returned.
	 ...
	 * @return The certification for the given JarEntry, or JarEntryCertification.NONE if
	 * no certification is found.
