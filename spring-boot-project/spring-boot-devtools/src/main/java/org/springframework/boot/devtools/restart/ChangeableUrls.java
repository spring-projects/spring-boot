...
final class ChangeableUrls implements Iterable<URL> {
	...
	/**
	 * Returns an array of URLs from the given ClassLoader. If the ClassLoader is an
	 * instance of URLClassLoader, it returns the URLs from the URLClassLoader. Otherwise,
	 * it splits the class path obtained from the runtime MXBean and converts each path to
	 * a URL.
	 ...
	 * @return A list of URLs from the classpath of the JAR file's manifest, or an empty
	 * list if not found.
	 * @throws IllegalStateException If failed to read the Class-Path attribute from the
	 * manifest.
	 */
	...
	/**
	 * Retrieves a list of URLs from the Class-Path attribute in the manifest file of a
	 * JAR file.
