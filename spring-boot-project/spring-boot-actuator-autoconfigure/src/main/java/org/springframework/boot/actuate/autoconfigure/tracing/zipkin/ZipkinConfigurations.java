...
class ZipkinConfigurations {
	...
	static class UrlConnectionSenderConfiguration {
		...
		/**
		 * Creates a new instance of URLConnectionSender if no other bean of type Sender
		 * is available. Uses the provided ZipkinProperties and ZipkinConnectionDetails to
		 * configure the sender. Sets the connect timeout and read timeout based on the
		 * properties. Sets the endpoint based on the connection details.
	...
	static class RestTemplateSenderConfiguration {
		...
		/**
		 * Creates a {@link ZipkinRestTemplateSender} bean if no other bean of type
		 * {@link Sender} is present.
		 * @param properties the {@link ZipkinProperties} containing the configuration
		 * properties for Zipkin
		 * @param customizers the {@link ZipkinRestTemplateBuilderCustomizer} objects to
		 * customize the {@link RestTemplateBuilder}
		 * @param connectionDetailsProvider the {@link ZipkinConnectionDetails} provider
		 * to get the connection details for Zipkin
		 ...
		 * @param customizers ObjectProvider of ZipkinRestTemplateBuilderCustomizer to
		 * provide customizers.
	...
	static class WebClientSenderConfiguration {
		...
		/**
		 * Creates a ZipkinWebClientSender bean if no other bean of type Sender is
		 * present. Uses the provided ZipkinProperties, ZipkinWebClientBuilderCustomizer,
		 * and ZipkinConnectionDetails to configure and create a ZipkinWebClientSender
		 * instance. If no ZipkinConnectionDetails bean is available, it creates a new
		 * instance using the provided ZipkinProperties. Uses the WebClient.Builder to
		 * build a WebClient instance with customizations from the
		 * ZipkinWebClientBuilderCustomizer beans. Returns the created
		 * ZipkinWebClientSender with the configured span endpoint and timeouts.
		 * @param properties the ZipkinProperties used for configuration
		 * @param customizers the ZipkinWebClientBuilderCustomizer beans for customizing
		 * the WebClient.Builder
		 * @param connectionDetailsProvider the ZipkinConnectionDetails bean provider for
	...
	static class ReporterConfiguration {
		 ...
		 *
		 * It is also annotated with @ConditionalOnMissingBean(Reporter.class) to ensure
		 * that this bean is only created if there is no other bean of type Reporter
		 * already present in the application context.
		 *
		 * Additionally, it is annotated with @ConditionalOnBean(Sender.class) to ensure
		 * that this bean is only created if there is a bean of type Sender already
		 * present in the application context.
		 *
		 * The method takes a Sender object and a BytesEncoder object as parameters.
		 *
		 * It returns an AsyncReporter object that is built using the provided Sender and
		 * BytesEncoder objects.
	...
	static class BraveConfiguration {
		...
		/**
		 * Creates a ZipkinSpanHandler bean if there is no existing bean of the same type,
		 * a bean of type Reporter<Span> exists, and tracing is enabled.
