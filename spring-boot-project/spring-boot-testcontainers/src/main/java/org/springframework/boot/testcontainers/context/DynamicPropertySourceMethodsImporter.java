...
class DynamicPropertySourceMethodsImporter {
	...
	void registerDynamicPropertySources(BeanDefinitionRegistry beanDefinitionRegistry, Class<?> definitionClass) {
		...
		registerDynamicPropertySourceMethods(beanDefinitionRegistry, definitionClass, methods);
	}
	...
	private static void registerDynamicPropertySourceMethods(BeanDefinitionRegistry beanDefinitionRegistry,
			Class<?> definitionClass, Set<Method> methods) {
		String beanName = DynamicPropertySourceMethods.class.getName() + " for " + definitionClass.getName();
		if (!beanDefinitionRegistry.containsBeanDefinition(beanName)) {
			RootBeanDefinition bd = new RootBeanDefinition(DynamicPropertySourceMethods.class);
			bd.setInstanceSupplier(() -> new DynamicPropertySourceMethods(definitionClass, methods));
			bd.setRole(BeanDefinition.ROLE_INFRASTRUCTURE);
			bd.setAutowireCandidate(false);
			bd.setAttribute(DynamicPropertySourceMethods.class.getName(), true);
			beanDefinitionRegistry.registerBeanDefinition(beanName, bd);
		}
	}

	private record DynamicPropertySourceMethods(Class<?> definitionClass, Set<Method> methods) {
	}

	/**
	 * {@link BeanRegistrationExcludeFilter} to exclude
	 * {@link DynamicPropertySourceMethods} from AOT bean registrations.
	 */
	static class DynamicPropertySourceMethodsBeanRegistrationExcludeFilter implements BeanRegistrationExcludeFilter {

		@Override
		public boolean isExcludedFromAotProcessing(RegisteredBean registeredBean) {
			return registeredBean.getMergedBeanDefinition().hasAttribute(DynamicPropertySourceMethods.class.getName());
		}
	 ...
	 *
	 */
	static class DynamicPropertySourceMethodsBeanFactoryInitializationAotProcessor
		...
		public BeanFactoryInitializationAotContribution processAheadOfTime(
			...
			Map<String, DynamicPropertySourceMethods> metadata = beanFactory
				.getBeansOfType(DynamicPropertySourceMethods.class, false, false);
			if (metadata.isEmpty()) {
				return null;
			}
		...
		private static final class AotContribution implements BeanFactoryInitializationAotContribution {
			...
			private final Map<String, DynamicPropertySourceMethods> metadata;

			private AotContribution(Map<String, DynamicPropertySourceMethods> metadata) {
			...
			private GeneratedMethod generateMethods(GenerationContext generationContext,
				...
				return generatedClass.getMethods().add("registerDynamicPropertySource", (code) -> {
					code.addJavadoc("Registers {@code @DynamicPropertySource} properties for a class '$T'",
							definitionClass);
			...
			// Generate @DynamicPropertySource methods
			...
			private static GeneratedMethod generateMethod(GenerationContext generationContext,
				...
				return generatedClass.getMethods().add(method.getName(), (code) -> {
					code.addJavadoc("Registers {@code @DynamicPropertySource} properties for a method '$T.$L'",
					...
					if (isMethodAccessible(generatedClass, method)) {
						...
						// ReflectionTestUtils can be used here as
						// @DynamicPropertyRegistry in a spring-test.
