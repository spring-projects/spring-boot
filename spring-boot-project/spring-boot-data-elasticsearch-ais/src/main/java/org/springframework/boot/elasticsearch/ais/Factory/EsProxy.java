...
public class EsProxy implements InvocationHandler {
  ...
  private Logger log = LoggerFactory.getLogger(EsProxy.class);
  ...
  public Object invoke(Object o, Method method, Object[] objects) throws Throwable {
    ...
    try {
      ...
      if (typesReturn.length > 0) {
        ...
        return restPerform.execute(requestMethod, index, script, Class.forName(returnClass));
      }
    } catch (ClassCastException e) {
      Type type = method.getReturnType();
      returnClass = type.getTypeName();
      if (returnClass.equals(String.class.getTypeName())) {
        return restPerform.executeString(requestMethod, index, script);
      } else {
