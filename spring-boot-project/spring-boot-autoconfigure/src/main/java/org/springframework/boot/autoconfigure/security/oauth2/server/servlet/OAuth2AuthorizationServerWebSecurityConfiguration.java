...
class OAuth2AuthorizationServerWebSecurityConfiguration {
	...
	/**
	 * Configures the security filter chain for the authorization server. This filter
	 * chain is responsible for handling security aspects of the authorization server. The
	 * filter chain is applied with the highest precedence.
	 ...
	 * This filter chain is responsible for handling authentication and authorization for
	 * incoming HTTP requests. The filter chain is ordered with the BASIC_AUTH_ORDER to
	 * ensure that basic authentication is applied before other authentication mechanisms.
