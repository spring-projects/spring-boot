...
public abstract class AbstractErrorWebExceptionHandler implements ErrorWebExceptionHandler, InitializingBean {
	 ...
	 * present or is not a valid boolean value.
	 */
	 ...
	 * @return the HTML escaped string representation of the input object, or null if the
	 * input is null
	 */
	...
	/**
	 * Handle the given throwable and generate an appropriate response. If the response
	 * has already been committed or the throwable is a disconnected client error, the
	 * throwable is propagated as is. Otherwise, the error information is stored in the
	 * error attributes and a routing function is used to determine the appropriate error
	 * handler. If no error handler is found, the throwable is propagated. If an error
	 * handler is found, the request is handled by the error handler and the response is
	 * logged. Finally, the response is written to the exchange.
	...
	/**
	 * Writes the response to the given ServerWebExchange using the provided
	 * ServerResponse.
