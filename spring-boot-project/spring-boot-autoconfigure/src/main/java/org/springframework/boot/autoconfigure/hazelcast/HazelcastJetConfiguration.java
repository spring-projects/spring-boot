...
/**
 * <p>Auto-configuration for Hazelcast Jet.
 * </p>
 * <p>This is activated when Jet classes are present, the beans that would be
 * auto-configured are not already present, and Jet configuration is found.
 * </p>
 * <p>Four rules apply:
 * </p>
 * <ul>
 * <li><b>1st equal</b> If only a Jet client configuration object exists,
 * create a Jet client.
 * </li>
 * <li><b>1st equal</b> If only a Jet server configuration object exists,
 * create a Jet server.
 * </li>
 * <li><b>3rd equal</b> If only a Jet client configuration XML file exists,
 * create a Jet client.
 * </li>
 * <li><b>3rd equal</b> If Jet server configuration XML file exists,
 * create a Jet server.
 * </li>
 * </ul>
 * <p>A {@link JetInstance} always contains a {@link HazelcastInstance}. This
 * {@link HazelcastInstance} is also exposed as a bean to stop subsequent
 * auto-configuration from creating another {@link HazelcastInstance}.
 * Usually a JVM process will not want more than one {@link HazelcastInstance}.
 * </p>
 *
...
public class HazelcastJetConfiguration {
	...
	private static final Log logger =
			LogFactory.getLog(HazelcastJetConfiguration.class);
	...
	/* ----- Hazelcast Jet -----
	 * Four mutually exclusive creators
	 */

	/**
	 * <p>Jet creation #1.
	 * </p>
	 * <p>If a client configuration object is available, and not
	 * a server configuration object, create a Jet client from
	 * it.</p>
	 */
	@Configuration
	 ...
	 * <p>Jet creation #2.
	 * </p>
	 * <p>The opposite of Jet creation #1. If a server
	 * configuration object is available and not a client
	 * configuration object, create a Jet server.
	 * </p>
	 */
	@Configuration
	 ...
	 * <p>Jet creation #3.
	 * </p>
	 * <p>If only the client configuration file is
	 * given, no server configuration files, create
	 * a Jet client from it</p>
	 */
	@Configuration
	...
	static class JetClientFileBasedConfiguration {
		...
		public JetInstance jetInstance(HazelcastProperties hazelcastProperties)
			...
			logger.trace("JetClientFileBasedConfiguration.JetInstance, resource=" + config);
			if (config != null) {
	 ...
	 * <p>Jet creation #4.
	 * </p>
	 * <p>If either of the Jet server configuration files are
	 * given, no client configuration, create a Jet server with that
	 * file.
	 * </p>
	 */
	@Configuration
	...
	static class JetServerFileBasedConfiguration {
		...
		public JetInstance jetInstance(HazelcastProperties hazelcastProperties)
			...
			Resource config = hazelcastProperties.resolveConfigLocation();
			logger.info("JetServerFileBasedConfiguration.JetInstance, resource=" + config);
			if (config != null && config.isFile()) {
				JetConfig jetConfig =
						JetConfig.loadFromStream(config.getInputStream());
	...
	/**
	 * <p>To avoid later auto-configuration, make it clear there is a {@link HazelcastInstance}.
	 * created internally.</p>
	 * <p>If the {@link JetInstance} is a server, the {@link HazelcastInstance} will be a server.
	 * If the {@link JetInstance} is a client, the {@link HazelcastInstance} will be a client.
	 * </p>
	 */
	@Configuration
	static class HazelcastInstanceFromJetInstance {
		/**
		 * <p>Expose inner {@link HazelcastInstance} as a bean.</p>
		 *
	...
	// 'hazelcast.xml'
	static class IMDGServerXMLAvailableCondition extends HazelcastConfigResourceCondition {
	...
	/**
	 * <p>"{@code hazelcast-client.xml}" but not
	 * ( "{@code hazelcast.xml}" or "{@code hazelcast-jet.xml}" ).
	 * </p>
	 */
	...
	/**
	 * <p>"{@code hazelcast-jet.xml}" or "{@code hazelcast.xml}" but not
