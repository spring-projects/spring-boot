...
public class GraphQlAutoConfiguration {
	...
	/**
	 * Constructs a new instance of GraphQlAutoConfiguration with the specified
	 * beanFactory.
	 ...
	 *
	 * This method uses the provided resourcePatternResolver, properties,
	 * exceptionResolvers, subscriptionExceptionResolvers, instrumentations,
	 * wiringConfigurers, sourceCustomizers, and typeDefinitionConfigurers to configure
	 * and build the GraphQlSource bean.
	 *
	 ...
	 *
	 * The exceptionResolvers, subscriptionExceptionResolvers, and instrumentations are
	 * ordered and added to the GraphQlSource builder.
	 *
	 * If the inspection is enabled in the properties, the schema mappings are logged.
	 *
	 * If the introspection is disabled in the properties, the enableIntrospection method
	 * is used to configure the runtime wiring.
	 *
	 * The ConnectionTypeDefinitionConfigurer is added to the GraphQlSource builder to
	 * configure the type definitions.
	 *
	 * The wiringConfigurers and sourceCustomizers are ordered and used to configure the
	 * runtime wiring and customize the GraphQlSource builder respectively.
	 * @param resourcePatternResolver The resource pattern resolver used to resolve the
	 * schema locations.
	 * @param properties The GraphQlProperties used to configure the GraphQlSource.
	 * @param exceptionResolvers The exception resolvers used to handle data fetcher
	 * exceptions.
	 * @param subscriptionExceptionResolvers The exception resolvers used to handle
	 * subscription exceptions.
	 * @param instrumentations The instrumentations used to instrument the GraphQL
	 * execution.
	 * @param wiringConfigurers The runtime wiring configurers used to configure the
	 * runtime wiring.
	 * @param sourceCustomizers The source customizers used to customize the GraphQlSource
	 * builder.
	 * @param typeDefinitionConfigurers The type definition configurers used to configure
	...
	/**
	 * Resolves the schema resources using the given resource pattern resolver and
	 * pattern.
	...
	/**
	 * Creates a new instance of BatchLoaderRegistry if no other bean of type
	 * BatchLoaderRegistry is present in the application context.
	...
	/**
	 * Creates an instance of ExecutionGraphQlService if no other bean of the same type is
	 * present.
	...
	/**
	 * Creates and configures an instance of {@link AnnotatedControllerConfigurer}. This
	 * bean is conditional on the absence of another bean of the same type.
	...
	/**
	 * Returns the DataFetcherExceptionResolver obtained from the provided
	 * AnnotatedControllerConfigurer.
	 * @param annotatedControllerConfigurer the AnnotatedControllerConfigurer used to
	 * obtain the DataFetcherExceptionResolver
	 * @return the DataFetcherExceptionResolver obtained from the
	 * AnnotatedControllerConfigurer
	 */
	@Bean
	...
	static class GraphQlDataAutoConfiguration {
		 ...
		 * method. It is also annotated with @ConditionalOnMissingBean to specify that
		 * this bean should only be created if there is no existing bean of the same type.
		 * The encoding cursor strategy is used to determine the cursor strategy for
		 * pagination using scroll positions. The cursor strategy is created using the
		 * ScrollPositionCursorStrategy and the base64 cursor encoder.
