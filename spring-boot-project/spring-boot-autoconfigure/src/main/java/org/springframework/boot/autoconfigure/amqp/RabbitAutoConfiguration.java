...
public class RabbitAutoConfiguration {
	...
	protected static class RabbitConnectionFactoryCreator {
		...
		/**
		 * Constructs a new RabbitConnectionFactoryCreator with the specified
		 * RabbitProperties.
		 * @param properties the RabbitProperties to be used by the
		...
		/**
		 * Creates a RabbitConnectionDetails bean if no other bean of type
		 * RabbitConnectionDetails is present.
		...
		/**
		 * Creates a RabbitConnectionFactoryBeanConfigurer object with the given
		 * parameters. This method is annotated with @Bean and @ConditionalOnMissingBean,
		 * indicating that it will be used to create a bean if no other bean of the same
		 * type is present.
		 * @param resourceLoader The resource loader used to load resources.
		 * @param connectionDetails The RabbitConnectionDetails object containing the
		 * connection details.
		 * @param credentialsProvider The CredentialsProvider object providing the
		 * credentials for the connection.
		 * @param credentialsRefreshService The CredentialsRefreshService object used to
		 * refresh the credentials.
		 * @param sslBundles The SslBundles object containing the SSL bundles for the
		 * connection.
		 * @return The RabbitConnectionFactoryBeanConfigurer object created with the given
		 * parameters.
		 */
		@Bean
		...
		/**
		 * Configures the CachingConnectionFactory with the provided
		 * RabbitConnectionDetails and ConnectionNameStrategy. If a ConnectionNameStrategy
		 * is not provided, the default one will be used.
		 * @param connectionDetails the RabbitConnectionDetails containing the connection
		 * details
		 * @param connectionNameStrategy the ConnectionNameStrategy to be used for
		 * generating connection names
		 * @return the CachingConnectionFactoryConfigurer configured with the provided
		 * details
		 */
		@Bean
		@ConditionalOnMissingBean
		...
		/**
		 * Creates a CachingConnectionFactory for RabbitMQ if no ConnectionFactory bean is
		 * present.
		 * @param rabbitConnectionFactoryBeanConfigurer The
		 * RabbitConnectionFactoryBeanConfigurer used to configure the
		 * RabbitConnectionFactoryBean.
		 * @param rabbitCachingConnectionFactoryConfigurer The
		 * CachingConnectionFactoryConfigurer used to configure the
		 * CachingConnectionFactory.
		 * @param connectionFactoryCustomizers The ObjectProvider of
		 * ConnectionFactoryCustomizer used to customize the ConnectionFactory.
	...
	protected static class RabbitTemplateConfiguration {
		...
		/**
		 * Configures the RabbitTemplate with the provided RabbitProperties,
		 * MessageConverter, and RabbitRetryTemplateCustomizer. If no MessageConverter is
		 * provided, the default one will be used. If no RabbitRetryTemplateCustomizer is
		 * provided, no customizations will be applied to the RetryTemplate.
		 * @param properties The RabbitProperties to configure the RabbitTemplate.
		 * @param messageConverter The MessageConverter to be used by the RabbitTemplate.
		 * If not provided, the default one will be used.
		 * @param retryTemplateCustomizers The RabbitRetryTemplateCustomizer(s) to
		 * customize the RetryTemplate used by the RabbitTemplate. If not provided, no
		...
		/**
		 * Creates a RabbitTemplate bean if there is a single candidate ConnectionFactory
		 * bean and no RabbitOperations bean is present.
		 * @param configurer The RabbitTemplateConfigurer used to configure the
		 * RabbitTemplate.
		 * @param connectionFactory The ConnectionFactory used by the RabbitTemplate.
		 * @param customizers The ObjectProvider of RabbitTemplateCustomizer used to
		 * customize the RabbitTemplate.
		...
		/**
		 * Creates an instance of AmqpAdmin if there is a single candidate for
		 * ConnectionFactory and the property "spring.rabbitmq.dynamic" is either not
		 * present or set to true. If an instance of AmqpAdmin is already present, this
	...
	protected static class MessagingTemplateConfiguration {
		...
		/**
		 * Creates a RabbitMessagingTemplate bean if a single candidate RabbitTemplate
		 * bean is available.
		 * @param rabbitTemplate the RabbitTemplate bean to be used by the
