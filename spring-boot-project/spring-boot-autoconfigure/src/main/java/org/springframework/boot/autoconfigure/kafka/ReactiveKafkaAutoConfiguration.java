...
/**
 * {@link EnableAutoConfiguration Auto-configuration} for the Reactive client of Apache Kafka.
 *
 * @author Almog Tavor
 * @since 2.7.0
 */
...
public class ReactiveKafkaAutoConfiguration {
    ...
    public <K, V> ReceiverOptions<K, V> receiverOptions() {
        ...
        Optional<Duration> closeTimeout = Optional.ofNullable(this.reactiveKafkaProperties.getReceiver().getCloseTimeout());
        if (closeTimeout.isPresent()) {
            receiverOptions = receiverOptions.closeTimeout(closeTimeout.get());
        }
		Optional<Duration> pollTimeout = Optional.ofNullable(this.reactiveKafkaProperties.getReceiver().getPollTimeout());
