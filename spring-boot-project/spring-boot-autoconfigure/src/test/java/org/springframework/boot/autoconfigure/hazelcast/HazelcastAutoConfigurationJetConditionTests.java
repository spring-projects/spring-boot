...
public class HazelcastAutoConfigurationJetConditionTests {
	...
	private static final String[] HAZELCAST_FILES = {
			HAZELCAST_CLIENT_XML_FILE, HAZELCAST_JET_XML_FILE, HAZELCAST_XML_FILE
	};

	private static final Map<String, String> temporaryFilenames = new HashMap<>();
	private static final ClassLoader classLoader =
			HazelcastAutoConfigurationJetConditionTests.class.getClassLoader();
	...
	/**
	 * <p>The various {@code @Test} methods in this class try combinations
	 * of presence and absence of Hazelcast files.
	 * </p>
	 * <p>To avoid side effects, look for these files in "{@code target/test-classes}"
	 * first and temporarily rename them.
	 * </p>
	 * <p>For instance, "{@code src/test/resources/hazelcast.xml}".
	 * </p>
	 *
		...
		for (String originalFilename : HAZELCAST_FILES) {
			...
			if (url != null) {
				...
				String temporaryFilename =
						...
						+ File.separator
						+ HazelcastAutoConfigurationJetConditionTests.class.getSimpleName()
						+ "."
						+ originalFilename;
				...
				if (!success) {
					throw new Exception("Cannot rename " + file.getAbsolutePath() + " to " + temporaryFilename);
				}
				else {
					temporaryFilenames.put(destination.getAbsolutePath(), file.getAbsolutePath());
				}
			}
		}
	}
	...
	/**
	 * <p>Reverse the file renaming done by {@code @BeforeClass}
	 * </p>
	 *
	...
	public static void tearDownAfterClass() throws Exception {
		for (Entry<String, String> entry : temporaryFilenames.entrySet()) {
			...
			if (!success) {
				throw new Exception("Cannot rename " + temporaryFilename + " to " + originalFilename);
			}
		}
	}

	/**
	 * <p>Create a classloader that looks in JUnit's temporary folder.
	 * </p>
	 *
	...
	public void setUp() throws Exception {
		ClassLoader applicationClassLoader = this.applicationContextRunner.getClass().getClassLoader();
	...
	public void hazelcast_xml_file() throws Exception {
		...
		this.getApplicationContextRunner()
		.run(this.beanFound("found", true,
				HAZELCAST_XML_FILE, JET_SERVER_CONFIGURATION_BEAN_NAME))
		.run(this.beanFound("not found", false,
				HAZELCAST_CLIENT_XML_FILE, HAZELCAST_JET_XML_FILE, JET_CLIENT_CONFIGURATION_BEAN_NAME));
	}
	...
	public void hazelcast_client_xml_file() throws Exception {
		...
		.run(this.beanFound("not found", false,
				HAZELCAST_XML_FILE, HAZELCAST_JET_XML_FILE, JET_SERVER_CONFIGURATION_BEAN_NAME));
	}
	...
	public void hazelcast_jet_xml_file() throws Exception {
		...
		.run(this.beanFound("not found", false,
				HAZELCAST_XML_FILE, HAZELCAST_CLIENT_XML_FILE, JET_CLIENT_CONFIGURATION_BEAN_NAME));
	}
	...
	public void hazelcast_xml_and_hazelcast_client_xml_file() throws Exception {
		...
		this.getApplicationContextRunner()
		.run(this.beanFound("found", true,
				HAZELCAST_XML_FILE, HAZELCAST_CLIENT_XML_FILE))
		.run(this.beanFound("not found", false,
				HAZELCAST_JET_XML_FILE, JET_CLIENT_CONFIGURATION_BEAN_NAME, JET_SERVER_CONFIGURATION_BEAN_NAME));
	}
	...
	public void hazelcast_client_xml_and_hazelcast_jet_xml_file() throws Exception {
		...
		.run(this.beanFound("not found", false,
				HAZELCAST_XML_FILE, JET_CLIENT_CONFIGURATION_BEAN_NAME, JET_SERVER_CONFIGURATION_BEAN_NAME));
	}

	@Test
	public void hazelcast_xml_and_hazelcast_client_xml_and_hazelcast_jet_xml_file() throws Exception {
		this.createFilesOnClasspath(HAZELCAST_XML_FILE, HAZELCAST_CLIENT_XML_FILE, HAZELCAST_JET_XML_FILE);
	...
	/**
	 * <p>Create empty files on the classpath in the temporary folder.
	 * </p>
	 *
	...
	/**
	 * <p>Test if Spring's application context has a bean with a given name.
	 * </p>
	 ...
	 */
	private ContextConsumer<AssertableApplicationContext> beanFound(String description, boolean expected, String... beanNames) {
		return (assertableApplicationContext) -> {
			if (expected) {
				for (String beanName : beanNames) {
					Assertions.assertThat(assertableApplicationContext).as(description).hasBean(beanName);
				}
			}
			else {
				for (String beanName : beanNames) {
					Assertions.assertThat(assertableApplicationContext).as(description).doesNotHaveBean(beanName);
				}
			}
		};
	}
	...
	/**
	 * <p>Build an application context runner to use in tests
	 * that attempts to pick up the configuration classes.
	 * If their conditions aren't met they won't be added.
	 * </p>
	 * @return Application context runner for one Junit test
	 */
	private ApplicationContextRunner getApplicationContextRunner() {
		return this.applicationContextRunner
					.withClassLoader(this.testClassLoader)
					// Config to test
					.withUserConfiguration(HazelcastJetConfiguration.ClientXMLAvailableCondition.class)
					.withUserConfiguration(HazelcastJetConfiguration.IMDGServerXMLAvailableCondition.class)
					.withUserConfiguration(HazelcastJetConfiguration.JetServerXMLAvailableCondition.class)
					.withUserConfiguration(HazelcastJetConfiguration.NoClientXMLAvailableCondition.class)
					.withUserConfiguration(HazelcastJetConfiguration.NoServerXMLAvailableCondition.class)
					.withUserConfiguration(HazelcastJetConfiguration.ServerXMLAvailableCondition.class)
					.withUserConfiguration(HazelcastJetConfiguration.JetClientConfigurationAvailableCondition.class)
					.withUserConfiguration(HazelcastJetConfiguration.JetServerConfigurationAvailableCondition.class)
					// Test helper config
					.withUserConfiguration(HazelcastAutoConfigurationJetConditionTests.HazelcastXml.class)
					.withUserConfiguration(HazelcastAutoConfigurationJetConditionTests.HazelcastClientXml.class)
					.withUserConfiguration(HazelcastAutoConfigurationJetConditionTests.HazelcastJetXml.class)
					.withUserConfiguration(HazelcastAutoConfigurationJetConditionTests.JetClientConfiguration.class)
					.withUserConfiguration(HazelcastAutoConfigurationJetConditionTests.JetServerConfiguration.class);
	}
