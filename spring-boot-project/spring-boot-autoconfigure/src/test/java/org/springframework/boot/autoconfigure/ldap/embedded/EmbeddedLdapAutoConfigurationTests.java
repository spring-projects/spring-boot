...
public class EmbeddedLdapAutoConfigurationTests {
	...
	public void testRandomPortWithEnvironment() {
		this.contextRunner.withPropertyValues("spring.ldap.embedded.base-dn[0]:dc=spring,dc=org")
				.run(context -> {
	...
	public void testRandomPortWithValueAnnotation() {
		...
		assertThat(connection.getConnectedPort()).isEqualTo(context.getEnvironment()
				.getProperty("local.ldap.port", Integer.class));
	}
	...
	public void testQueryEmbeddedLdap() {
		this.contextRunner.withPropertyValues("spring.ldap.embedded.base-dn[0]:dc=spring,dc=org")
				.withConfiguration(AutoConfigurations.of(LdapAutoConfiguration.class, LdapDataAutoConfiguration.class))
				.run(context -> {
