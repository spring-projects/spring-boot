	...
	void autoConfigurationShouldConfigureResourceServerUsingJwkSetUriAndIssuerUri() throws Exception {
		...
		this.contextRunner
				...
				.run((context) -> {
					...
					DelegatingOAuth2TokenValidator<Jwt> jwtValidator = (DelegatingOAuth2TokenValidator) ReflectionUtils
							.tryToReadFieldValue(NimbusReactiveJwtDecoder.class.getDeclaredField("jwtValidator"),
									reactiveJwtDecoder)
							.get();
					Collection<OAuth2TokenValidator<Jwt>> tokenValidators = (Collection<OAuth2TokenValidator<Jwt>>) ReflectionUtils
							.tryToReadFieldValue(
