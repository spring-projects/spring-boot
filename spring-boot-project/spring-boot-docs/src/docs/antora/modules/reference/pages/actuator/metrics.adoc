...
- xref:actuator/metrics.adoc#actuator.metrics.export.dash0[]
...
[[actuator.metrics.export.dash0]]
=== Dash0

Dash0 is 100% {url-micrometer-docs-implementations}/otlp[OpenTelemetry] compatible, so
you can provide the location of the https://www.dash0.com/[Dash0 metric endpoint] to use by using the `otlp` exporter.
Additionally you need to provide your `AUTH_TOKEN` as `Authorization` header.
...
management:
  otlp:
    metrics:
      export:
        url: "https://ingress.eu-west-1.aws.dash0-dev.com/v1/metrics"
        headers:
