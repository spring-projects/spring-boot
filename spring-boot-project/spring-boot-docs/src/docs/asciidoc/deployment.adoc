...
If you are running your application from a container, you can use an runnable jar, but it is also often an advantage to explode it and run it in a different way.
...
Spring Boot's runnable jars are ready-made for most popular cloud PaaS (Platform-as-a-Service) providers.
...
Ideally, your application, like a Spring Boot runnable jar, has everything that it needs to run packaged within it.
...
You can deploy stand-alone runnable jar applications as well as traditional `.war` packaged applications.
...
You can configure a sleep command in a preStop handler to avoid requests being routed to a pod that has already begun shutting down.
...
Amazon Web Services offers multiple ways to install Spring Boot-based applications, either as traditional web applications (war) or as runnable jar files with an embedded web server.
The options include:
...
https://boxfuse.com/[Boxfuse] works by turning your Spring Boot runnable jar or war into a minimal VM image that can be deployed unchanged either on VirtualBox or on AWS.
...
If your runnable jar or war contains an https://boxfuse.com/docs/payloads/springboot.html#configuration[`application-boxfuse.properties`] file, Boxfuse bases its configuration on the properties it contains.
...
In addition to running Spring Boot applications by using `java -jar`, it is also possible to make fully runnable applications for Unix systems.
A fully runnable jar can be run like any other runnable binary or it can be <<deployment-service,registered with `init.d` or `systemd`>>.
This makes it very easy to install and manage Spring Boot applications in common production environments.

CAUTION: Fully runnable jars work by embedding an extra script at the front of the file.
Currently, some tools do not accept this format, so you may not always be able to use this technique.
For example, `jar -xf` may silently fail to extract a jar or war that has been made fully runnable.
It is recommended that you make your jar or war fully runnable only if you intend to run it directly, rather than running it with `java -jar` or deploying it to a servlet container.

CAUTION: A zip64-format jar file cannot be made fully runnable.
Attempting to do so will result in a jar file that is reported as corrupt when runned directly or with `java -jar`.
A standard-format jar file that contains one or more zip64-format nested jars can be fully runnable.

To create a "`fully runnable`" jar with Maven, use the following plugin configuration:
...
If you configured Spring Boot's Maven or Gradle plugin to generate a <<deployment-install, fully runnable jar>>, and you do not use a custom `embeddedLaunchScript`, your application can be used as an `init.d` service.
...
When run as root, as is the case when root is being used to start an init.d service, the default runnable script runs the application as the user specified in the `RUN_AS_USER` environment variable.
...
In this case, the default runnable script runs the application as the `bootapp` user.
...
Firstly, configure its permissions so that it cannot be written and can only be read or run by its owner, as shown in the following example:
...
To customize written elements, use the `embeddedLaunchScriptProperties` option of the Spring Boot Maven plugin or the {spring-boot-gradle-plugin-docs}#packaging-runnable-configuring-launch-script[`properties` property of the Spring Boot Gradle plugin's `launchScript`].
...
| `JAVA_HOME`
| The location of the `java` runnable is discovered by using the `PATH` by default, but you can set it explicitly if there is an runnable file at `$JAVA_HOME/bin/java`.
