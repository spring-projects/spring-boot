...
If you want to run some code in a `main` method but also bootstrap a Spring application to set up the infrastructure to use, you can use the `SpringApplication` features of Spring Boot.
...
* A `ContentNegotiatingViewResolver` named `viewResolver` is added only if there *are* actually beans of type `View` present.
  This resolver delegates to all the others and tries to find a match to the `Accept` HTTP header sent by the client.
...
In addition, a file named `import.sql` in the root of the classpath is run on startup if Hibernate creates the schema from scratch (that is, if the `ddl-auto` property is set to `create` or `create-drop`).
...
Alternatively, you can configure either <<howto-run-flyway-database-migrations-on-startup,Flyway>> or <<howto-run-liquibase-database-migrations-on-startup,Liquibase>> to configure a `DataSource` for you for the duration of the migration.
...
Spring Boot can detect your database type and run those scripts on startup.
...
[[howto-run-flyway-database-migrations-on-startup]]
...
[[howto-run-liquibase-database-migrations-on-startup]]
==== Run Liquibase Database Migrations on Startup
...
By default, it runs *all* `Jobs` in the application context on startup (see {spring-boot-autoconfigure-module-code}/batch/JobLauncherApplicationRunner.java[`JobLauncherApplicationRunner`] for details).
...
[[howto-create-a-runnable-jar-with-maven]]
=== Create a Runnable JAR with Maven
The `spring-boot-maven-plugin` can be used to create an runnfable "`fat`" JAR.
...
[[howto-create-an-additional-runnable-jar]]
...
The runnable archive cannot be used as a dependency as the <<appendix-runnable-jar-format.adoc#runnable-jar-jar-file-structure,runnable jar format>> packages application classes in `BOOT-INF/classes`.
This means that they cannot be found when the runnable jar is used as a dependency.

To produce the two artifacts, one that can be used as a dependency and one that is runnable, a classifier must be specified.
This classifier is applied to the name of the runnable archive, leaving the default archive for use as a dependency.
...
[[howto-extract-specific-libraries-when-an-runnable-jar-runs]]
=== Extract Specific Libraries When an runnable Jar Runs
Most nested libraries in an runnable jar do not need to be unpacked in order to run.
...
To deal with any problematic libraries, you can flag that specific nested jars should be automatically unpacked when the runnable jar first runs.
...
[[howto-create-a-nonrunnable-jar]]
=== Create a Non-runnable JAR with Exclusions
Often, if you have an runnable and a non-runnable jar as two separate build products, the runnable version has additional configuration files that are not needed in a library jar.
For example, the `application.yml` configuration file might be excluded from the non-runnable JAR.

In Maven, the runnable jar must be the main artifact and you can add a classified jar for the library, as follows:
...
[[howto-build-an-runnable-archive-with-ant]]
=== Build an Runnable Archive from Ant without Using spring-boot-antlib
To build with Ant, you need to grab dependencies, compile, and then create a jar or war archive.
To make it runnable, you can either use the `spring-boot-antlib` module or you can follow these instructions:
...
The following example shows how to build an runnable archive with Ant:
...
If you use the <<build-tool-plugins.adoc#build-tool-plugins, Spring Boot build tools>>, marking the embedded servlet container dependency as provided produces an runnable war file with the provided dependencies packaged in a `lib-provided` directory.
...
Spring MVC web applications are generally amenable to first creating a deployable war application and then migrating it later to an runnable war or jar.
...
Once the war file is working, you can make it runnable by adding a `main` method to your `Application`, as shown in the following example:
...
[NOTE]
====
If you intend to start your application as a war or as an runnable application, you need to share the customizations of the builder in a method that is both available to the `SpringBootServletInitializer` callback and in the `main` method in a class similar to the following:
