...
If you need a log level other than `INFO`, you can set it, as described in <<features.logging.log-levels>>.
...
NOTE: Spring Boot provides numerous `FailureAnalyzer` implementations, and you can <<howto.adoc#howto.application.failure-analyzer,add your own>>.
...
To do so, you need to <<features.external-config,enable the `debug` property>> or <<features.logging.log-levels,enable `DEBUG` logging>> for `org.springframework.boot.autoconfigure.logging.ConditionEvaluationReportLoggingListener`.
...
See _<<features.external-config>>_ for details.
...
An application is considered live as soon as the context has been refreshed, see <<features.spring-application.application-events-and-listeners, Spring Boot application lifecycle and related Application Events>>.
...
An application is considered ready as soon as application and command-line runners have been called, see <<features.spring-application.application-events-and-listeners, Spring Boot application lifecycle and related Application Events>>.
...
. An `ApplicationReadyEvent` is sent after any <<features.spring-application.command-line-runner,application and command-line runners>> have been called.
...
Consider using <<features.spring-application.command-line-runner,application and command-line runners>> instead.
...
Property values can be injected directly into your beans by using the `@Value` annotation, accessed through Spring's `Environment` abstraction, or be <<features.external-config.typesafe-configuration-properties,bound to structured objects>> through `@ConfigurationProperties`.
...
. <<features.external-config.files,Application properties>> packaged inside your jar (`application.properties` and YAML variants).
. <<features.external-config.files.profile-specific,Profile-specific application properties>> packaged inside your jar (`application-\{profile}.properties` and YAML variants).
. <<features.external-config.files,Application properties>> outside of your packaged jar (`application.properties` and YAML variants).
. <<features.external-config.files.profile-specific,Profile-specific application properties>> outside of your packaged jar (`application-\{profile}.properties` and YAML variants).
...
TIP: Use the prefix `optional:` if the <<features.external-config.files.optional-prefix,locations are optional>> and you don't mind if they don't exist.
...
See <<features.external-config.typesafe-configuration-properties.relaxed-binding.environment-variables>> for details.
...
You can use this prefix with the `spring.config.location` and `spring.config.additional-location` properties, as well as with <<features.external-config.files.importing, `spring.config.import`>> declarations.
...
If you've already directly <<features.external-config.files.importing,imported>> a profile specific property files then it won't be imported a second time.
...
By default you can import Java Properties, YAML and "`<<features.external-config.files.configtree, configuration trees>>`".
...
For the first case, you can import the YAML or Properties file directly using `spring.config.import` as described <<features.external-config.files.importing,above>>.
...
See the _<<howto.adoc#howto.properties-and-configuration.short-command-line-arguments>>_ how-to for details.
...
See the <<features.external-config.files.activation-properties, next section>> for details.
...
See <<howto.adoc#howto.application.customize-the-environment-or-application-context>> for details.
...
For more details see the "`<<features.external-config.typesafe-configuration-properties>>`" section below.
...
TIP: See also the <<features.external-config.typesafe-configuration-properties.vs-value-annotation,differences between `@Value` and type-safe configuration properties>>.
...
| Upper case format with underscore as the delimiter (see <<features.external-config.typesafe-configuration-properties.relaxed-binding.environment-variables>>).
| Numeric values surrounded by underscores (see <<features.external-config.typesafe-configuration-properties.relaxed-binding.environment-variables>>)
...
| <<features.external-config.typesafe-configuration-properties.relaxed-binding,Relaxed binding>>
| Yes
...
`SpEL` expressions from  <<features.external-config.files,application property files>> are not processed at time of parsing these files and populating the environment.
...
Profile groups, which are described in the <<features.profiles.groups,next section>> can also be used to add active profiles if a given profile is active.
...
See "<<features.external-config.files.profile-specific>>" for details.
...
If you need to configure logging for a class, you can use <<features.external-config.application-json, the `SPRING_APPLICATION_JSON`>> variable.
...
[TIP]
====
If you want to use a placeholder in a logging property, you should use <<features.external-config.files.property-placeholders,Spring Boot's syntax>> and not the syntax of the underlying framework.
...
Several configuration properties are provided for <<howto.adoc#howto.spring-mvc.customize-jackson-objectmapper,customizing the configuration of the `ObjectMapper`>>.
...
* Support for serving static resources, including support for WebJars (covered <<features.developing-web-applications.spring-mvc.static-content,later in this document>>)).
* Automatic registration of `Converter`, `GenericConverter`, and `Formatter` beans.
* Support for `HttpMessageConverters` (covered <<features.developing-web-applications.spring-mvc.message-converters,later in this document>>).
* Automatic registration of `MessageCodesResolver` (covered <<features.developing-web-applications.spring-mvc.message-codes,later in this document>>).
* Static `index.html` support.
* Automatic use of a `ConfigurableWebBindingInitializer` bean (covered <<features.developing-web-applications.spring-mvc.binding-initializer,later in this document>>).
...
There are several <<features.developing-web-applications.embedded-container.jsp-limitations, known limitations>> when using them with embedded servlet containers.
...
* Configuring codecs for `HttpMessageReader` and `HttpMessageWriter` instances (described <<features.developing-web-applications.spring-webflux.httpcodecs,later in this document>>).
* Support for serving static resources, including support for WebJars (described <<features.developing-web-applications.spring-mvc.static-content,later in this document>>).
...
You can also leverage <<features.developing-web-applications.spring-mvc.json,Boot's custom JSON serializers and deserializers>>.
...
You can also provide your own HTML templates to display errors (see the <<features.developing-web-applications.spring-webflux.error-handling.error-pages,next section>>).
...
For example, it cannot scan for endpoints in a package found in a <<deployment.adoc#deployment.installing, fully executable jar file>> or in `WEB-INF/classes` when running an executable war file.
...
TIP: To see the order of every `Filter` in your application, enable debug level logging for the `web` <<features.logging.log-groups,logging group>> (`logging.level.web=debug`).
...
* <<howto.adoc#howto.webserver.configure-ssl,SSL>>
* <<howto.adoc#howto.webserver.enable-response-compression,HTTP compression>>
...
For instance, <<howto.adoc#howto.webserver.configure-access-logs,access logs>> can be configured with specific features of the embedded servlet container.
...
* Creating a custom `error.jsp` page does not override the default view for <<features.developing-web-applications.spring-mvc.error-handling,error handling>>.
  <<features.developing-web-applications.spring-mvc.error-handling.error-pages,Custom error pages>> should be used instead.
...
You can learn more about the resource configuration on the client side in the <<features.webclient.runtime, WebClient Runtime section>>.
...
Check out the <<features.json.jackson,Jackson support section>> to know more about customization possibilities.
...
TIP: See <<howto.adoc#howto.data-access.configure-custom-datasource,the "`How-to`" section>> for more advanced examples, typically to take full control over the configuration of the DataSource.
...
TIP: The "`How-to`" section includes a <<howto.adoc#howto.data-initialization, section on how to initialize a database>>.
...
These are the standard options that work regardless of <<features.sql.datasource.connection-pool, the actual implementation>>.
...
See the "`<<howto.adoc#howto.data-access.separate-entity-definitions-from-spring-configuration>>`" how-to.
...
TIP: The "`How-to`" section includes a <<howto.adoc#howto.data-initialization.using-basic-sql-scripts, section on how to initialize a database>>.
...
==== Embedded Database Support
Similarly to <<features.sql.datasource.embedded,the JDBC support>>, Spring Boot can automatically configure an embedded database for reactive usage.
You need not provide any connection URLs.
...
In the presence of `spring-data-elasticsearch` and the required dependencies for using a `WebClient` (typically `spring-boot-starter-webflux`), Spring Boot can also auto-configure a <<features.nosql.elasticsearch.connecting-using-reactive-rest,ReactiveElasticsearchClient>> and a `ReactiveElasticsearchTemplate` as beans.
...
If you do not add any specific cache library, Spring Boot auto-configures a <<features.caching.provider.simple,simple provider>> that uses concurrent maps in memory.
...
. <<features.caching.provider.generic,Generic>>
. <<features.caching.provider.jcache,JCache (JSR-107)>> (EhCache 3, Hazelcast, Infinispan, and others)
. <<features.caching.provider.ehcache2,EhCache 2.x>>
. <<features.caching.provider.hazelcast,Hazelcast>>
. <<features.caching.provider.infinispan,Infinispan>>
. <<features.caching.provider.couchbase,Couchbase>>
. <<features.caching.provider.redis,Redis>>
. <<features.caching.provider.caffeine,Caffeine>>
. <<features.caching.provider.simple,Simple>>

TIP: It is also possible to _force_ a particular cache provider by setting the configprop:spring.cache.type[] property.
Use this property if you need to <<features.caching.provider.none,disable caching altogether>> in certain environment (such as tests).
...
TIP: Spring Boot has <<features.hazelcast,general support for Hazelcast>>.
...
==== Hazelcast
Spring Boot has <<features.hazelcast,general support for Hazelcast>>.
...
==== Couchbase
If Spring Data Couchbase is available and Couchbase is <<features.nosql.couchbase,configured>>, a `CouchbaseCacheManager` is auto-configured.
...
Spring Boot is configuring that builder to share HTTP resources, reflect codecs setup in the same fashion as the server ones (see <<features.developing-web-applications.spring-webflux.httpcodecs,WebFlux HTTP codecs auto-configuration>>), and more.
...
NOTE: Spring Boot also has <<features.caching.provider.hazelcast,explicit caching support for Hazelcast>>.
...
Spring Integration polling logic relies <<features.task-execution-and-scheduling,on the auto-configured `TaskScheduler`>>.
...
The annotation works by <<features.testing.spring-boot-applications.detecting-configuration,creating the `ApplicationContext` used in your tests through `SpringApplication`>>.
In addition to `@SpringBootTest` a number of other annotations are also provided for <<features.testing.spring-boot-applications.autoconfigured-tests,testing more specific slices>> of an application.
...
* `MOCK`(Default) : Loads a web `ApplicationContext` and provides a mock web environment.
  Embedded servers are not started when using this annotation.
	...
	It can be used in conjunction with <<features.testing.spring-boot-applications.with-mock-environment, `@AutoConfigureMockMvc` or `@AutoConfigureWebTestClient`>> for mock-based testing of your web application.
...
[NOTE]
====
If you use a <<features.testing.spring-boot-applications.autoconfigured-tests, test annotation to test a more specific slice of your application>>, you should avoid adding configuration settings that are specific to a particular area on the <<features.testing.spring-boot-applications.user-configuration-and-slicing, main method's application class>>.
...
As we <<features.testing.spring-boot-applications.detecting-configuration,have seen earlier>>, `@TestConfiguration` can be used on an inner class of a test to customize the primary configuration.
...
If your application expects <<features.spring-application.application-arguments,arguments>>, you can
...
TIP: If you want to focus only on the web layer and not start a complete `ApplicationContext`, consider <<features.testing.spring-boot-applications.spring-mvc-tests,using `@WebMvcTest` instead>>.
...
This means that, whilst you can test your MVC layer throws and handles exceptions as expected, you cannot directly test that a specific <<features.developing-web-applications.spring-mvc.error-handling.error-pages, custom error page>> is rendered.
...
The `@LocalServerPort` annotation can be used to <<howto.adoc#howto.webserver.discover-port,inject the actual port used>> into your test.
...
More details on how to use Spring Security's `MockMvc` support can be found in this _<<howto.adoc#howto.testing-with-spring-security>>_ how-to section.

TIP: Sometimes writing Spring MVC tests is not enough; Spring Boot can help you run <<features.testing.spring-boot-applications.with-running-server, full end-to-end tests with an actual server>>.
...
TIP: Sometimes writing Spring WebFlux tests is not enough; Spring Boot can help you run <<features.testing.spring-boot-applications.with-running-server, full end-to-end tests with an actual server>>.
...
(See "<<features.testing.spring-boot-applications.autoconfigured-spring-data-jpa>>".)
...
See the {spring-framework-docs}/testing.html#testcontext-tx-enabling-transactions[relevant section] in the Spring Framework Reference Documentation for more details.
If that is not what you want, you can disable transaction management for a test or for the whole test class as <<features.testing.spring-boot-applications.autoconfigured-jdbc,shown in the JDBC example>>.
...
(For more about using jOOQ with Spring Boot, see "<<features.sql.jooq>>", earlier in this chapter.)
...
TIP: Spring Framework 5.0 provides a new `WebTestClient` that works for <<features.testing.spring-boot-applications.spring-webflux-tests, WebFlux integration tests>> and both <<features.testing.spring-boot-applications.with-running-server, WebFlux and MVC end-to-end testing>>.
...
We first cover what you need to know to build your own auto-configuration and then we move on to the <<features.developing-auto-configuration.custom-starter,typical steps required to create a custom starter>>.
...
`@ConfigurationProperties` when used in combination with <<features.external-config.typesafe-configuration-properties.constructor-binding,`@ConstructorBinding`>> supports classes with immutable `val` properties as shown in the following example:
...
If you need the `Mockk` equivalent of the Mockito specific <<features.testing.spring-boot-applications.mocking-beans,`@MockBean` and `@SpyBean` annotations>>, you can use https://github.com/Ninja-Squad/springmockk[SpringMockK] which provides similar `@MockkBean` and `@SpykBean` annotations.
