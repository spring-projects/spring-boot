...
If you want to use Tomcat's MBeans, for example so that they can be used to expose metrics through Micrometer, you must use the configprop:server.tomcat.mbeanregistry.enabled[] property to do so, as shown in the following example:
