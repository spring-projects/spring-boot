...
Spring Boot also provides an optional <<build-tool-plugins.adoc#build-tool-plugins-maven-plugin, Maven plugin>> to create runnable jars.
...
Spring Boot provides a useful <<build-tool-plugins.adoc#build-tool-plugins-gradle-plugin, Gradle plugin>> that can be used to simplify dependency declarations and to create runnable jars.
...
On a Debian system, the system-wide scripts are in `/shell-completion/bash` and all scripts in that directory are run when a new shell starts.
...
[[getting-started-first-application-runnable-jar]]
=== Creating a Runnable Jar
We finish our example by creating a completely self-contained runnable jar file that we could run in production.
Runnable jars (sometimes called "`fat jars`") are archives containing your compiled classes along with all of the jar dependencies that your code needs to run.

.Runnable jars and Java
****
...
Spring Boot takes a <<appendix-runnable-jar-format.adoc#runnable-jar, different approach>> and lets you actually nest jars directly.
****

To create an runnable jar, we need to add the `spring-boot-maven-plugin` to our `pom.xml`.
