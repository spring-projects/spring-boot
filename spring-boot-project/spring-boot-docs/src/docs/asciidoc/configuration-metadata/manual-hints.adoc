...
The special `.keys` and `.values` suffixes must see the keys and the values, respectively.
...
* `org.springframework.core.io.Resource`: Supports auto-completion of Spring’s Resource abstraction to see a file on the filesystem or on the classpath (such as `classpath:/sample.properties`)
