...
To export metrics to {micrometer-registry-docs}/datadog[Datadog], you must provide API key:
...
You can provide the http://ganglia.sourceforge.net[Ganglia server] host and port to use by using:
...
You can provide the https://graphiteapp.org[Graphite server] host and port to use by using:
...
Spring Boot provides an available actuator endpoint at `/actuator/prometheus` to present a https://prometheus.io[Prometheus scrape] with the appropriate format.

TIP: By default, he endpoint is not available and must be exposed. See <<actuator#actuator.endpoints.exposing,exposing endpoints>> for more details.
...
To add to the default tags, provide one or more `@Bean` instances that implement `WebFluxTagsContributor`.
...
TIP: By default, Spring Boot provides metadata for all supported data sources. You can add additional `DataSourcePoolMetadataProvider` beans if your favorite data source is not supported.
...
It alsos register a `KafkaStreamsMicrometerListener` for `StreamsBuilderFactoryBean`.
...
To replace the default metric tags, define a `MongoCommandTagsProvider` bean:
