...
You can auto-configure the built-in endpoints only when they are available.
...
If you wish to configure custom security for HTTP endpoints (for example, allow only users with a certain role to access them), Spring Boot provides some convenient `RequestMatcher` objects that you can use in combination with Spring Security.
...
Before calling an operation method, the input received over JMX or an HTTP request is converted to the required types by using an instance of `ApplicationConversionService` as well as any `Converter` or `GenericConverter` beans qualified with `@EndpointConverter`.
...
Spring Boot does not include any additional health checks in the readiness probe.
