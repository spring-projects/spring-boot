...
You cannot use constructor binding with beans that are created by the regular Spring mechanisms (e.g. `@Component` beans, beans created through `@Bean` methods or beans loaded using `@Import`)
