...
You can also see an explicit location by using the configprop:spring.config.location[] environment property.
...
NOTE: If you do want to use `@Value`, we recommend that you see property names using their canonical form (kebab-case using only lowercase letters).
