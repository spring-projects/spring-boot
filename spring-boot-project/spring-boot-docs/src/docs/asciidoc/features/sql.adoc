...
These are the standard options that work regardless of <<features#features.sql.datasource.connection-pool, the actual implementation>>.
...
WARNING: If session in view is enabled, it keeps an entity manager active the whole web request, that uses an active database connection from the limited connection pool. In some cases, this behavior may use up all available connections, leading to connection unavailable errors.
...
==== Embedded Database Support
Similarly to <<features#features.sql.datasource.embedded,the JDBC support>>, Spring Boot can automatically configure an embedded database for reactive usage.
You need not provide any connection URLs.
