...
NOTE: You'll need to add `@ImportAutoConfiguration(ServiceConnectionAutoConfiguration.class)` if you are using <<features#features.testing.spring-boot-applications.autoconfigured-tests, test slices>>, unless your container is not spring bean but using testcontainers `@Testcontainers` and `@Container` instead.
You also need take other related annotations into account, for example you need add `@AutoConfigureTestDatabase(replace = NONE)` for `@DataJpaTest` to disable replacing containerized database with embedded database.
