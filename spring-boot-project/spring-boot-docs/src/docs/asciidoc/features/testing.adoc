...
Alternatively, additional auto-configurations can be added for any use of a slice annotation by registering them in a file stored in `META-INF/springboot` as shown in the following example:

.META-INF/springboot/org.springframework.boot.test.autoconfigure.jdbc.JdbcTest
[indent=0]
----
