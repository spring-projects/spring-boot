...
The plugins offer a variety of features, including the packaging of runnable jars.
...
The Spring Boot Maven Plugin provides Spring Boot support in Maven, letting you package runnable jar or war archives and run an application "`in-place`".
...
The Spring Boot Gradle Plugin provides Spring Boot support in Gradle, letting you package runnable jar or war archives, run Spring Boot applications, and use the dependency management provided by `spring-boot-dependencies`.
...
You can use the module to create runnable jars.
...
You can use the `exejar` task to create a Spring Boot runnable jar.
...
Runnable jars need to follow a specific format and certain entries need to be written in an uncompressed form (see the "`<<appendix-runnable-jar-format.adoc#runnable-jar, runnable jar format>>`" section in the appendix for details).
...
To repackage an existing archive so that it becomes a self-contained runnable archive, use `org.springframework.boot.loader.tools.Repackager`.
...
More technical details of the runnable jar format are covered in <<appendix-runnable-jar-format#runnable-jar,the appendix>>.
