...
==== Error Handling
By default, Spring Boot provides an `/error` {spring-boot-docs}/html/appendix-application-properties.html#server-properties[mapping that handles all errors] in a sensible way, and it is registered as a "`global`" error page in the servlet container.
