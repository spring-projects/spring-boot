...
==== Dockerfiles
While it is possible to convert a Spring Boot fat jar into a docker image with just a few lines in the Dockerfile, we will use the <<features.container-images.layering,layering feature>> to create an optimized docker image.
...
CAUTION: The `layertools` mode can not be used with a <<deployment.adoc#deployment.installing, fully executable Spring Boot archive>> that includes a launch script.
