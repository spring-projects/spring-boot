...
/**
 * Tests for {@link MyLegacyCookieProcessorConfiguration}.
 *
 * @author Andy Wilkinson
 */
class MyLegacyCookieProcessorConfigurationTests {
	...
	void cookieProcessorIsCustomized() {
		ServletWebServerApplicationContext applicationContext = (ServletWebServerApplicationContext) new SpringApplication(
				TestConfiguration.class, MyLegacyCookieProcessorConfiguration.class)
			.run();
		Context context = (Context) ((TomcatWebServer) applicationContext.getWebServer()).getTomcat()
			.getHost()
			.findChildren()[0];
