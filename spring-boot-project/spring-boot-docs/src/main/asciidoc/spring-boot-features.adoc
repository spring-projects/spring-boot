...
consumers and admins), the `prop.two` admin property to `second`, the `prop.three` consumer
property to `third` and the `prop.four` producer property to `fourth`.

When using the spring-kafka `JsonDeserializer`, you can also configure it using boot
properties:
