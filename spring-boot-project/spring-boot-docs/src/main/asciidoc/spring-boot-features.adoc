...
If https://jet.hazelcast.org/[Hazelcast Jet] is on the classpath, Spring Boot auto-configures
a `JetInstance` for your application. This includes a `HazelcastInstance`.

Here Spring Boot will look for the following:

* The presence of a `com.hazelcast.client.config.ClientConfig` bean.
* The presence of a `com.hazelcast.jet.config.JetConfig` bean.
* The `hazelcast.jet.config` system property.
* `hazelcast-jet.xml` in the working directory or the root of the classpath.
