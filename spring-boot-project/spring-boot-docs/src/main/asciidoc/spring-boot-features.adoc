...
and reference your listener(s) by using the `org.springframework.context.ApplicationListener`
key.
...
`spring.application.json` in a System property, as shown in the following example:
...
in `/` (and, at runtime, be appended with the names generated from `spring.config.name` before
...
When a collection is specified in multiple profiles, the one with
the highest priority (and only that one) is used:
...
is useful include dash-separated environment properties (for example, `context-path` binds to `contextPath`), and
...
can provide a `ConversionService` bean (with bean named `conversionService`) or custom
...
* https://github.com/spring-projects/spring-boot/blob/master/spring-boot-project/spring-boot/src/main/resources/org/springframework/boot/logging/logback/defaults.xml[Logback]
* https://github.com/spring-projects/spring-boot/blob/master/spring-boot-project/spring-boot/src/main/resources/org/springframework/boot/logging/log4j2/log4j2.xml[Log4j 2]
* https://github.com/spring-projects/spring-boot/blob/master/spring-boot-project/spring-boot/src/main/resources/org/springframework/boot/logging/java/logging-file.properties[Java Util logging]
...
{{dc-spring-boot}}/jackson/JsonObjectSerializer.html[JsonObjectSerializer]
and
{{dc-spring-boot}}/jackson/JsonObjectDeserializer.html[JsonObjectDeserializer]
...
{{spring-javadoc}}/validation/DefaultMessageCodesResolver.Format.html[`DefaultMessageCodesResolver.Format`]).
...
manually declare this filter when using JSPs. Other template engines are currently not automatically
...
https://docs.spring.io/spring-boot/docs/current-SNAPSHOT/api/org/springframework/boot/web/servlet/server/ConfigurableServletWebServerFactory.html[source code documentation]
for details.
...
`org.springframework.boot.autoconfigure.security` category is set to log `INFO`-level messages.
...
For instance, the following configuration creates `cache1` and `cache2` caches with a maximum
...
functional feel and is fully reactive. You can create your own client
...
applications. By default, Spring Boot creates an `MBeanServer` a bean with an ID of
...
See the
{{dc-spring-boot-test}}/mock/mockito/SpyBean.html[Javadoc]
for full details.
...
when it makes sense. The latter approach lets you enable it in one of your tests,
if necessary, with the  `@Import` annotation.
