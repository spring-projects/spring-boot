...
In the normal Spring way, you can use a `spring.profiles.active` `Environment` property
...
the root of the classpath or in a location specified by the Spring `Environment` property
`logging.config`.
...
`ResourceConfigCustomizer` for more advanced customizations.
...
this behavior, you may want to customize your `tmpwatch` configuration, so that
`tomcat.*` directories are not deleted or configure `server.tomcat.basedir`, so that
embedded Tomcat uses a different location.
...
functionality: creating `Repository` implementations directly from interfaces and using
...
connection pool], you could customize many additional settings:
...
configure and manage your DataSource using your Application Server's built-in features
...
the https://projects.spring.io/spring-data-jpa/[Spring Data JPA reference documentation].
...
instead of using `MongoDbFactory`. You can also declare your own `MongoDbFactory` or
`Mongo` bean if you want to take complete control of establishing the MongoDB connection.
...
technologies, refer to the https://projects.spring.io/spring-data-mongodb/[reference
documentation].
...
https://pivotal.io/pivotal-gemfire#details[Pivotal Gemfire] data management
...
you have custom attributes, you can use `spring.ldap.embedded.validation.schema` to
...
explicitly. Otherwise, the default bootstrap is used.
...
startup by setting the `spring.cache.cache-names` property and can be customized by one
...
`spring-boot-starter-jta-bitronix` starter to add the appropriate Bitronix dependencies
...
JBoss. You can use the `spring-boot-starter-jta-narayana` starter to add the appropriate
...
resides). You can customize this directory by setting a `spring.jta.log-dir` property in
your `application.properties` file. Properties starting with
`spring.jta.narayana.properties` can also be used to customize the Narayana
...
for testing more specific slices of an application. You can find more detail later in
this document.
...
test that Spring MVC controllers are mapping URLs correctly, and you don't want to
involve database calls in those tests, or you _might want_ to test JPA entities, and you
...
that the driver is quit after each test and that a new instance is injected. If you do
...
you can use `@AutoconfigureTestDatabase` to override those settings.
