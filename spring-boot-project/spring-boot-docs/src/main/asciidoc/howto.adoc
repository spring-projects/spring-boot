...
[[howto-disable-register-shutdown-hook]]
=== Disable Register Shutdown Hook
When the Spring Boot starts, it will register a shutdown hook with the JVM runtime by default. To disable this behavior configure the `RegisterShutdownHook` in your `application.properties`, as shown in the following example:
