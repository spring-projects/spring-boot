...
----
	...
	# EMBEDDED CASSANDRA ({sc-spring-boot-autoconfigure}/cassandra/embedded/EmbeddedCassandraProperties.{sc-ext}[EmbeddedCassandraProperties])
	spring.cassandra.embedded.authenticator=org.apache.cassandra.auth.AllowAllAuthenticator # Authentication backend, implementing IAuthenticator; used to identify users.
    spring.cassandra.embedded.authorizer=org.apache.cassandra.auth.AllowAllAuthorizer # Authorization backend, implementing IAuthority; used to limit access/provide permissions.
    spring.cassandra.embedded.auto-snapshot=true # Whether or not a snapshot is taken of the data before keyspace truncation or dropping of column families.
    spring.cassandra.embedded.back-pressure-strategy.parameters= # Parameters.
    spring.cassandra.embedded.back-pressure-strategy.target-class= # Target class.
    spring.cassandra.embedded.broadcast-address= # Address to broadcast to other Cassandra nodes Leaving this blank will set it to the same value as listenAddress.
    spring.cassandra.embedded.broadcast-rpc-address= # RPC address to broadcast to drivers and other Cassandra nodes.
    spring.cassandra.embedded.cas-contention-timeout=1s # How long a coordinator should continue to retry a CAS operation that contends with other proposals for the same row.
    spring.cassandra.embedded.cdc-enabled=false # Change-data-capture logs.
    spring.cassandra.embedded.cdc-raw-directory= # The directory where the CDC log is stored.
    spring.cassandra.embedded.client-encryption.algorithm= # Set the algorithm.
    spring.cassandra.embedded.client-encryption.cipher-suites= # Add cipher suites.
    spring.cassandra.embedded.client-encryption.enabled=false # To enable, set to true.
    spring.cassandra.embedded.client-encryption.keystore= # The location of a Java keystore (JKS) suitable for use with Java Secure Socket Extension (JSSE), which is the Java version of the Secure Sockets Layer (SSL), and Transport Layer Security (TLS) protocols. The keystore contains the private key used to encrypt outgoing messages.
    spring.cassandra.embedded.client-encryption.keystore-password= # Password for the keystore.
    spring.cassandra.embedded.client-encryption.optional=false # If enabled and optional is set to true encrypted and unencrypted connections are handled.
    spring.cassandra.embedded.client-encryption.protocol= # Set the protocol.
    spring.cassandra.embedded.client-encryption.require-client-auth=false # Enables or disables certificate authentication.
    spring.cassandra.embedded.client-encryption.require-endpoint-verification=false # Enables or disables host name verification.
    spring.cassandra.embedded.client-encryption.store-type= # Set the store type.
    spring.cassandra.embedded.client-encryption.truststore= # Location of the truststore containing the trusted certificate for authenticating remote servers.
    spring.cassandra.embedded.client-encryption.truststore-password= # Password for the truststore.
    spring.cassandra.embedded.cluster-name= # The name of the cluster. This is mainly used to prevent machines in one logical cluster from joining another.
    spring.cassandra.embedded.commit-failure-policy=stop # Policy for commit disk failures.
    spring.cassandra.embedded.commit-log-compression.parameters= # Parameters.
    spring.cassandra.embedded.commit-log-compression.target-class= # Target class.
    spring.cassandra.embedded.commit-log-directory= # The directory where the commit log is stored.
    spring.cassandra.embedded.commit-log-sync=periodic # Policy for commit log sync.
    spring.cassandra.embedded.commit-log-sync-batch= # When in batch mode, Cassandra won’t ack writes until the commit log has been fsynced to disk. It will wait commitLogSyncBatch between fsyncs
    spring.cassandra.embedded.commit-log-sync-period=10s # When in periodic mode, the CommitLog is simply synced every commitLogSyncPeriod.
    spring.cassandra.embedded.concurrent-counter-writes=32 # Set concurrent counter writes.
    spring.cassandra.embedded.concurrent-materialized-view-writes=32 # For materialized view writes, as there is a read involved, so this should be limited by the less of concurrent reads or concurrent writes.
    spring.cassandra.embedded.concurrent-reads=32 # Set concurrent reads.
    spring.cassandra.embedded.concurrent-writes=32 # Set concurrent writes.
    spring.cassandra.embedded.counter-write-request-timeout=5s # How long the coordinator should wait for counter writes to complete.
    spring.cassandra.embedded.credentials-update-interval=2s # Refresh interval for credentials cache (if enabled). After this interval, cache entries become eligible for refresh.
    spring.cassandra.embedded.credentials-validity=2s # Validity period for credentials cache. This cache is tightly coupled to the provided PasswordAuthenticator implementation of IAuthenticator. If another IAuthenticator implementation is configured, this cache will not be automatically used and so the following settings will have no effect.
    spring.cassandra.embedded.data-directories= # Directories where Cassandra should store data on disk.
    spring.cassandra.embedded.disk-access-mode=auto # Policy for data disk access mode.
    spring.cassandra.embedded.disk-failure-policy=ignore # Policy for data disk failures.
    spring.cassandra.embedded.disk-optimization-strategy=ssd # The strategy for optimizing disk read.
    spring.cassandra.embedded.endpoint-snitch=org.apache.cassandra.locator.SimpleSnitch # Set to a class that implements the IEndpointSnitch interface. Cassandra uses the snitch to locate nodes and route requests.
    spring.cassandra.embedded.hints-compression.parameters= # Parameters.
    spring.cassandra.embedded.hints-compression.target-class= # Target class.
    spring.cassandra.embedded.hints-directory= # Directory where Cassandra should store hints.
    spring.cassandra.embedded.hints-flush-period=10s # The number of milliseconds Cassandra waits before flushing hints from internal buffers to disk.
    spring.cassandra.embedded.internode-authenticator=org.apache.cassandra.auth.AllowAllInternodeAuthenticator # Internode authentication backend, implementing IInternodeAuthenticator; used to allow/disallow connections from peer nodes.
    spring.cassandra.embedded.internode-compression=none # Compression controls whether traffic between nodes is compressed.
    spring.cassandra.embedded.listen-address=localhost # Address or interface to bind to and tell other Cassandra nodes to connect to. Set listenAddress OR listenInterface, not both.
    spring.cassandra.embedded.listen-interface= # Set listenAddress OR listenInterface, not both. Interfaces must correspond to a single address, IP aliasing is not supported.
    spring.cassandra.embedded.listen-interface-prefer-ipv6=false # If you choose to specify the interface by name and the interface has an ipv4 and an ipv6 address you can specify which should be chosen using listenInterfacePreferIpv6. If false the first ipv4 address will be used. If true the first ipv6 address will be used. Defaults to false preferring ipv4. If there is only one address it will be selected regardless of ipv4/ipv6.
    spring.cassandra.embedded.listen-on-broadcast-address=false # When using multiple physical network interfaces, set this to true to listen on broadcastAddress in addition to the listenAddress, allowing nodes to communicate in both interfaces.
    spring.cassandra.embedded.memtable-allocation-type=heap_buffers # Policy for memory table allocation type.
    spring.cassandra.embedded.partitioner=org.apache.cassandra.dht.Murmur3Partitioner # The partitioner is responsible for distributing groups of rows (by partition key) across nodes in the cluster. Besides Murmur3Partitioner, partitioner included for backwards compatibility include RandomPartitioner, ByteOrderedPartitioner, and OrderPreservingPartitioner.
    spring.cassandra.embedded.permissions-update-interval=2s # Refresh interval for permissions cache (if enabled). After this interval, cache entries become eligible for refresh.
    spring.cassandra.embedded.permissions-validity=2s # Validity period for permissions cache. Will be disabled automatically for AllowAllAuthorizer.
    spring.cassandra.embedded.port=0 # Port for the CQL native transport to listen for clients on.
    spring.cassandra.embedded.native-transport-enabled=true # Start the native transport or not.
    spring.cassandra.embedded.port-ssl= # Enabling native transport encryption.
    spring.cassandra.embedded.range-request-timeout=10s # How long the coordinator should wait for seq or index scans to complete.
    spring.cassandra.embedded.read-request-timeout=5s # How long the coordinator should wait for read operations to complete.
    spring.cassandra.embedded.request-scheduler=org.apache.cassandra.scheduler.NoScheduler # Class that implements IRequestScheduler.
    spring.cassandra.embedded.request-scheduler-id=keyspace # Policy for request scheduler id.
    spring.cassandra.embedded.request-timeout=10s # The default timeout for other, miscellaneous operations.
    spring.cassandra.embedded.role-manager=org.apache.cassandra.auth.CassandraRoleManager # Part of the Authentication & Authorization backend, implementing IRoleManager.
    spring.cassandra.embedded.roles-update-interval=2s # Refresh interval for roles cache (if enabled). After this interval, cache entries become eligible for refresh.
    spring.cassandra.embedded.roles-validity=2s # Validity period for roles cache Will be disabled automatically for AllowAllAuthenticator.
    spring.cassandra.embedded.rpc-address=localhost # The address or interface to bind the native transport server to. Set rpcAddress OR rpcInterface, not both.
    spring.cassandra.embedded.rpc-interface= # Interfaces must correspond to a single address, IP aliasing is not supported. Set rpcAddress OR rpcInterface, not both.
    spring.cassandra.embedded.rpc-interface-prefer-ipv6=false # If you choose to specify the interface by name and the interface has an ipv4 and an ipv6 address you can specify  which should be chosen using rpcInterfacePreferIpv6. If false the first ipv4 address will be used. If true the first ipv6 address will be used. Defaults to false preferring ipv4. If there is only one address it will be selected regardless of ipv4/ipv6.
    spring.cassandra.embedded.rpc-keepalive=true # Enable or disable keepalive on rpc/native connections.
    spring.cassandra.embedded.rpc-port=0 # port for Thrift to listen for clients on.
    spring.cassandra.embedded.rpc-transport-enabled=true # Start RPC transport or not.
    spring.cassandra.embedded.rpc-type=sync # Type of RPC server.
    spring.cassandra.embedded.saved-caches-directory= # The directory location where table key and row caches are stored.
    spring.cassandra.embedded.seed-provider.parameters= # Parameters.
    spring.cassandra.embedded.seed-provider.target-class=org.apache.cassandra.locator.SimpleSeedProvider # Target class.
    spring.cassandra.embedded.server-encryption.algorithm= # Set the algorithm.
    spring.cassandra.embedded.server-encryption.cipher-suites= # Added cipher suites.
    spring.cassandra.embedded.server-encryption.internode-encryption= # Policy for internode encryption.
    spring.cassandra.embedded.server-encryption.keystore= # The location of a Java keystore (JKS) suitable for use with Java Secure Socket Extension (JSSE), which is the Java version of the Secure Sockets Layer (SSL), and Transport Layer Security (TLS) protocols. The keystore contains the private key used to encrypt outgoing messages.
    spring.cassandra.embedded.server-encryption.keystore-password= # Password for the keystore.
    spring.cassandra.embedded.server-encryption.protocol= # Set the protocol.
    spring.cassandra.embedded.server-encryption.require-client-auth=false # Enables or disables certificate authentication.
    spring.cassandra.embedded.server-encryption.require-endpoint-verification=false # Enables or disables host name verification.
    spring.cassandra.embedded.server-encryption.store-type= # Set the store type.
    spring.cassandra.embedded.server-encryption.truststore= # Location of the truststore containing the trusted certificate for authenticating remote servers.
    spring.cassandra.embedded.server-encryption.truststore-password= # Password for the truststore.
    spring.cassandra.embedded.slow-query-log-timeout=500ms # How long before a node logs slow queries. Select queries that take longer than this timeout to execute, will generate an aggregated log message, so that slow queries can be identified. Set this value to zero to disable slow query logging.
    spring.cassandra.embedded.startup-timeout=1m # Startup timeout.
    spring.cassandra.embedded.storage-port=0 # TCP port, for commands and data.
    spring.cassandra.embedded.storage-ssl-port=0 # SSL port, for encrypted communication.
    spring.cassandra.embedded.truncate-request-timeout=1m # How long the coordinator should wait for truncates to complete.
    spring.cassandra.embedded.user-function-timeout-policy=die # Policy for user function timeout policy.
    spring.cassandra.embedded.working-directory= # Working directory.
    spring.cassandra.embedded.write-request-timeout=2s # How long the coordinator should wait for writes to complete.
    spring.cassandra.embedded.arguments= # Additional arguments which should be associated with cassandra process.
