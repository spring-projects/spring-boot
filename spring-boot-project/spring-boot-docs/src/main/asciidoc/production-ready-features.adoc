...
[[production-ready-metrics-webservices-clients]]
==== WebServices Client Metrics
Spring Boot Actuator manages the instrumentation of  `WebServiceTemplate`.
For that, you have to get injected with an auto-configured `WebServiceTemplateBuilder`
and use it to create `WebServiceTemplate`.
It is also possible to apply manually the `MetricsWebServiceTemplateCustomizer` customizer.

By default, metrics are generated with the name, `webservices.client.requests`. The name can be
customized by setting the `management.metrics.web-services.client.requests-metric-name` property.

By default, metrics generated by an instrumented client are tagged with the
following information:

* `uri`,  the request's URI
* `status`, the response's code (for example, `OK`, `IO_ERROR`, `CLIENT_ERROR`, `SERVER_ERROR`).
* `action`, the `SOAP` action.

To customize the tags, and depending on your choice of client, you can provide
a `@Bean` that implements `WebServiceTemplateExchangeTagsProvider`.
There are convenience static functions in the `WebServiceTemplateExchangeTags`.
