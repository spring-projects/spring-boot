...
class DockerComposeLifecycleManager {
	 ...
	 * @param serviceReadinessChecks the service readiness checks for Docker Compose
	 * services
	 */
	...
	/**
	 * Starts the Docker Compose services based on the configuration properties. If AOT
	 * processing or native images are enabled, Docker Compose support is disabled. If
	 * Docker Compose support is not enabled, the method returns. If Docker Compose
	 * support is skipped based on the configured skip conditions, the method returns.
	 * Retrieves the Docker Compose file and active profiles. Initializes the
	 * DockerCompose instance with the compose file and active profiles. If no services
	 * are defined in the Docker Compose file with the active profiles, a warning is
	 * logged and the method returns. Retrieves the lifecycle management, start, stop, and
	 * readiness configurations from the properties. Retrieves the list of running
	 * services from the DockerCompose instance. If lifecycle management requires starting
	 * the services and no services are currently running, the start command is applied to
	 * the DockerCompose instance. If the readiness configuration is set to
	 * ONLY_IF_STARTED, it is changed to ALWAYS. If lifecycle management requires stopping
	 * the services, a shutdown handler is added to apply the stop command with the
	 * configured timeout. If there are already Docker Compose services running, the
	 * method logs a message and skips the startup process. Filters out any ignored
	 * services from the list of running services. If the readiness configuration is set
	 * to ALWAYS or null, waits until the relevant services are ready. Publishes a
	 * DockerComposeServicesReadyEvent with the relevant services.
	 */
	void start() {
	...
	/**
	 * Retrieves the Docker Compose file to be used for the lifecycle management. If a
	 * file path is specified in the properties, it will be used. Otherwise, the method
	 * will search for a Docker Compose file in the working directory. If no Docker
	 * Compose file is found, an exception will be thrown.
	...
	/**
	 * Returns a DockerCompose object based on the provided DockerComposeFile and active
	 * profiles.
	 * @param composeFile the DockerComposeFile to use for creating the DockerCompose
	 * object
	 * @param activeProfiles the set of active profiles to use for creating the
	 * DockerCompose object
	 * @return the DockerCompose object created using the provided DockerComposeFile and
	 * active profiles
	 */
