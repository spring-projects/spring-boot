...
public enum TextOutputFormat implements Producible<TextOutputFormat> {
	...
	CONTENT_TYPE_004(TextFormat.CONTENT_TYPE_004) {
		...
		/**
		 * Writes the given metric family samples to the provided writer using the
		 * TextFormat version 0.0.4 format.
		 ...
		 * @throws IOException If an I/O error occurs while writing the metric family
		 * samples.
		 */
		@Override
	...
	CONTENT_TYPE_OPENMETRICS_100(TextFormat.CONTENT_TYPE_OPENMETRICS_100) {
		...
		/**
		 * Writes the given metric samples to the provided writer in OpenMetrics 1.0.0
		 * format.
