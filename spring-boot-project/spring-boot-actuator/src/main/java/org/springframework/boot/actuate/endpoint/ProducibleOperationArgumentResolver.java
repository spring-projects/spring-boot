...
public class ProducibleOperationArgumentResolver implements OperationArgumentResolver {
	 ...
	 * @throws ClassCastException if the resolved object cannot be cast to the specified
	 * class
	 */
	...
	/**
	 * Returns the Enum value that corresponds to the given MimeType, based on the list of
	 * values and their associated MimeType. If the MimeType is a wildcard type and a
	 * wildcard subtype, the default value is returned. Otherwise, it iterates through the
	 * list of values and checks if the MimeType is compatible with the produced MimeType
	 * of each value. If a compatible value is found, it is returned. If no compatible
	 ...
	 * @return the Enum value that corresponds to the given MimeType, or null if no match
	 * is found
	 */
	 ...
	 * @throws IllegalStateException if multiple default values are declared in the given
	 * type
	 */
