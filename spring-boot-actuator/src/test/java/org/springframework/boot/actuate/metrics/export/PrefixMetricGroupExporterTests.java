...
public class PrefixMetricGroupExporterTests {
	...
	private final PrefixMetricGroupExporter exporter = new PrefixMetricGroupExporter(this.reader, this.writer);

	@Test
	public void prefixedMetricsCopied() {
		this.reader.set("foo",
				Arrays.<Metric<?>>asList(new Metric<Number>("bar", 2.3), new Metric<Number>("spam", 1.3)));
	...
	public void countersIncremented() {
		...
		this.reader.set("counter", Collections.<Metric<?>>singletonList(new Metric<Number>("counter.foo.bar", 1)));
	...
	public void unprefixedMetricsNotCopied() {
		this.reader.set("foo",
				Arrays.<Metric<?>>asList(new Metric<Number>("foo.bar", 2.3), new Metric<Number>("foo.spam", 1.3)));
	...
	public void onlyPrefixedMetricsCopied() {
		this.reader.set("foo",
				Arrays.<Metric<?>>asList(new Metric<Number>("foo.bar", 2.3), new Metric<Number>("foo.spam", 1.3)));
		this.reader.set("foobar", Collections.<Metric<?>>singletonList(new Metric<Number>("foobar.spam", 1.3)));
		this.exporter.setGroups(Collections.singleton("foo"));
		this.exporter.export();
		assertThat(Iterables.collection(this.writer.groups())).hasSize(1);
	}
