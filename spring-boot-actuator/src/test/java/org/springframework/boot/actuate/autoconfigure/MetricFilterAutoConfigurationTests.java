...
public class MetricFilterAutoConfigurationTests {
	...
	public void defaultMetricFilterAutoConfigurationProperties() {
		...
		assertThat(properties.getGaugeSubmissions()).containsExactly(MetricsFilterSubmission.MERGED);
		assertThat(properties.getCounterSubmissions()).containsExactly(MetricsFilterSubmission.MERGED);
	}
	...
	public void recordsHttpInteractions() throws Exception {
		...
		request.setAttribute(HandlerMapping.BEST_MATCHING_PATTERN_ATTRIBUTE, "/test/path");
	...
	public void usesUnmappedForInteractionsWithNoBestMatchingPattern() throws Exception {
		...
		verify(context.getBean(GaugeService.class)).submit(eq("response.unmapped"), anyDouble());
		context.close();
	}
	...
	public void recordsHttpInteractionsWithRegexTemplateVariable() throws Exception {
		...
		verify(context.getBean(GaugeService.class)).submit(eq("response.templateVarRegexTest.someVariable"),
				anyDouble());
		context.close();
	}
	...
	public void recordsHttpInteractionsWithDoubleWildcardMapping() throws Exception {
		...
		verify(context.getBean(GaugeService.class)).submit(eq("response.doubleWildcardMapping.star-star.baz"),
				anyDouble());
		context.close();
	}
	...
	public void records302HttpInteractionsAsSingleMetric() throws Exception {
		AnnotationConfigApplicationContext context = new AnnotationConfigApplicationContext(Config.class,
				MetricFilterAutoConfiguration.class, RedirectFilter.class);
		MetricsFilter filter = context.getBean(MetricsFilter.class);
		MockMvc mvc = MockMvcBuilders.standaloneSetup(new MetricFilterTestController()).addFilter(filter)
				.addFilter(context.getBean(RedirectFilter.class)).build();
	...
	public void correctlyRecordsMetricsForDeferredResultResponse() throws Exception {
		...
		MvcResult result = mvc.perform(post("/create")).andExpect(status().isOk()).andExpect(request().asyncStarted())
				.andExpect(request().attribute(attributeName, is(notNullValue()))).andReturn();
		latch.countDown();
	...
	public void records5xxxHttpInteractionsAsSingleMetric() throws Exception {
		AnnotationConfigApplicationContext context = new AnnotationConfigApplicationContext(Config.class,
				MetricFilterAutoConfiguration.class, ServiceUnavailableFilter.class);
		MetricsFilter filter = context.getBean(MetricsFilter.class);
		MockMvc mvc = MockMvcBuilders.standaloneSetup(new MetricFilterTestController()).addFilter(filter)
	...
	public void doesNotRecordRolledUpMetricsIfConfigured() throws Exception {
		AnnotationConfigApplicationContext context = new AnnotationConfigApplicationContext();
		context.register(Config.class, MetricFilterAutoConfiguration.class);
		EnvironmentTestUtils.addEnvironment(context, "endpoints.metrics.filter.gauge-submissions=",
	...
	class MetricFilterTestController {
		...
		public String testTemplateVariableRegexResolution(@PathVariable String someVariable) {
			return someVariable;
		}
			...
			new Thread(new Runnable() {
				...
				public void run() {
					try {
						...
						result.setResult(new ResponseEntity<String>("Done", HttpStatus.CREATED));
					}
