...
/**
 * Tests for {@link NonHeapMemoryHeathIndicator}.
 *
 * @author Ben Hale
 */
public final class NonHeapMemoryHealthIndicatorTests {
	...
	public void setUp() throws Exception {
		...
		given(this.memoryMXBeanMock.getNonHeapMemoryUsage()).willReturn(this.memoryUsageMock);
		this.healthIndicator = new NonHeapMemoryHealthIndicator(
			createProperties(this.memoryMXBeanMock, THRESHOLD));
	}

	@Test
	public void nonHeapMemoryIsUp() {
	...
	@Test
	public void nonHeapMemoryIsUpNoMax() {
		given(this.memoryUsageMock.getInit()).willReturn(1L);
		given(this.memoryUsageMock.getUsed()).willReturn(2L);
		given(this.memoryUsageMock.getCommitted()).willReturn(3L);
	...
	@Test
	public void nonHeapMemoryIsDown() {
		given(this.memoryUsageMock.getInit()).willReturn(1L);
		given(this.memoryUsageMock.getUsed()).willReturn(2L);
		given(this.memoryUsageMock.getCommitted()).willReturn(3L);
	...
	private NonHeapMemoryHealthIndicatorProperties createProperties(MemoryMXBean memoryMXBean, double threshold) {
		NonHeapMemoryHealthIndicatorProperties properties = new NonHeapMemoryHealthIndicatorProperties();
