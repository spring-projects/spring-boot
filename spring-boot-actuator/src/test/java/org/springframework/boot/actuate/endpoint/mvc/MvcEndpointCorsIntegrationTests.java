...
public class MvcEndpointCorsIntegrationTests {
	...
	public void createContext() {
		...
		this.context.register(JacksonAutoConfiguration.class, HttpMessageConvertersAutoConfiguration.class,
				...
				AuditAutoConfiguration.class, JolokiaAutoConfiguration.class, WebMvcAutoConfiguration.class);
	}
	...
	public void settingAllowedOriginsEnablesCors() throws Exception {
		EnvironmentTestUtils.addEnvironment(this.context, "endpoints.cors.allowed-origins:foo.example.com");
		createMockMvc().perform(options("/beans").header("Origin", "bar.example.com")
	...
	public void requestsWithDisallowedHeadersAreRejected() throws Exception {
		EnvironmentTestUtils.addEnvironment(this.context, "endpoints.cors.allowed-origins:foo.example.com");
		createMockMvc().perform(options("/beans").header("Origin", "foo.example.com")
				.header(HttpHeaders.ACCESS_CONTROL_REQUEST_METHOD, "GET")
	...
	public void requestsWithDisallowedMethodsAreRejected() throws Exception {
		EnvironmentTestUtils.addEnvironment(this.context, "endpoints.cors.allowed-origins:foo.example.com");
		createMockMvc().perform(options("/health").header(HttpHeaders.ORIGIN, "foo.example.com")
	...
	public void jolokiaEndpointUsesGlobalCorsConfiguration() throws Exception {
		EnvironmentTestUtils.addEnvironment(this.context, "endpoints.cors.allowed-origins:foo.example.com");
		createMockMvc().perform(options("/jolokia").header("Origin", "bar.example.com")
				.header(HttpHeaders.ACCESS_CONTROL_REQUEST_METHOD, "GET")).andExpect(status().isForbidden());
