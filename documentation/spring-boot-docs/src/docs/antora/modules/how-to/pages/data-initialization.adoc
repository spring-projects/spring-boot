...
[[howto.data-initialization.security]]
== Initialize a Spring Security Database

Spring Security's JDBC-backed components store their state in SQL tables.
Spring Boot can create those tables for you, using the schema scripts shipped by Spring Security.

Note that Spring Boot only initializes the schema.
The components themselves, such as javadoc:org.springframework.security.provisioning.JdbcUserDetailsManager[], are left for you to declare as beans.

If you use an embedded database, the `users` and `authorities` tables are created by default.
You can also enable this for any database type, as shown in the following example:
...
Because `users` and `authorities` are common table names, you may want to switch this off explicitly by setting `spring.security.jdbc.initialize-schema` to `never`.

Spring Authorization Server's three tables are configured independently, so you can initialize only the ones you need:
...
Each of these defaults to `embedded`, so they are initialized automatically when an embedded database is in use.
