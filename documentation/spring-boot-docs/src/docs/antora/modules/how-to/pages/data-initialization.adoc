...
For the `users` and `authorities` tables, initialization is off by default, as those are common table names that may clash with your own schema.
Enable it explicitly, as shown in the following example:
...
Unlike the `users` table, these default to `embedded`, so they are initialized automatically when an embedded database is in use.
