...
By default, the handler does not create a `LongTaskTimer` for active observations.
To track observations while they are in progress, set configprop:management.metrics.observations.include-active-observation-long-task-timer[] to `true`.
The timer is named `<observation-name>.active` and includes only the low-cardinality tags available when the observation starts.
For example, the URI pattern for an HTTP server request is not yet available at that point, so its `uri` tag can be `UNKNOWN`.

This default differs from earlier Spring Boot versions, which enabled the active observation timer by default.
Set the new property to `true` to retain that behavior.
The deprecated configprop:management.metrics.observations.ignored-meters[] property continues to suppress the timer when it contains `long-task-timer`, even if the new property is set to `true`.
Remove that ignored entry when opting in.
