...
class AmqpAutoConfigurationTests {
	...
	@Test
	void autoConfigureCreatesDefaultContainerFactory() {
		this.contextRunner.run((context) -> assertThat(context).hasSingleBean(AmqpMessageListenerContainerFactory.class)
			.hasSingleBean(MethodAmqpMessageListenerContainerFactory.class)
			.hasBean(AmqpDefaultConfiguration.DEFAULT_AMQP_LISTENER_CONTAINER_FACTORY_BEAN_NAME));
	}

	@Test
	void autoConfigureConfiguresContainerFactoryWithDefaultSettings() {
		this.contextRunner.run((context) -> {
			AmqpMessageListenerContainerFactory factory = context.getBean(AmqpMessageListenerContainerFactory.class);
			TestAmqpListenerEndpoint endpoint = new TestAmqpListenerEndpoint();
			AmqpMessageListenerContainer container = factory.createContainer(endpoint);
			Listener listener = new AmqpProperties().getListener();
			assertThat(container).satisfies(listenerContainerConfiguration(listener.isAutoStartup(),
					listener.getConcurrency(), listener.getInitialCredits(), listener.getReceiveTimeout(),
					listener.getGracefulShutdownPeriod()));
			assertThat(endpoint.isDefaultRequeueRejected()).isEqualTo(listener.isDefaultRequeueRejected());
		});
	}

	@Test
	void autoConfigureConfiguresContainerFactoryWithOverrides() {
		this.contextRunner
			.withPropertyValues("spring.amqp.listener.auto-startup=false", "spring.amqp.listener.concurrency=3",
					"spring.amqp.listener.initial-credits=500", "spring.amqp.listener.receive-timeout=25s",
					"spring.amqp.listener.graceful-shutdown-period=35s",
					"spring.amqp.listener.default-requeue-rejected=false")
			.run((context) -> {
				AmqpMessageListenerContainerFactory factory = context
					.getBean(AmqpMessageListenerContainerFactory.class);
				TestAmqpListenerEndpoint endpoint = new TestAmqpListenerEndpoint();
				AmqpMessageListenerContainer container = factory.createContainer(endpoint);
				assertThat(container).satisfies(
						listenerContainerConfiguration(false, 3, 500, Duration.ofSeconds(25), Duration.ofSeconds(35)));
				assertThat(endpoint.isDefaultRequeueRejected()).isEqualTo(false);
			});
	}

	@Test
	void autoConfigurationCreatesContainerUsingAmqpMessageListenerContainerFactory() {
		AmqpMessageListenerContainerFactory factory = mock(AmqpMessageListenerContainerFactory.class);
		this.contextRunner
			.withBean(AmqpDefaultConfiguration.DEFAULT_AMQP_LISTENER_CONTAINER_FACTORY_BEAN_NAME,
					AmqpMessageListenerContainerFactory.class, () -> factory)
			.withUserConfiguration(TestListener.class)
			.run((context) -> then(factory).should().createContainer(any()));
	}

	private Consumer<AmqpMessageListenerContainer> listenerContainerConfiguration(boolean autoStartup, int concurrency,
			int initialCredit, Duration receiveTimeout, Duration gracefulShutdownPeriod) {
		return (container) -> assertThat(container).hasFieldOrPropertyWithValue("autoStartup", autoStartup)
			.hasFieldOrPropertyWithValue("consumersPerQueue", concurrency)
			.hasFieldOrPropertyWithValue("initialCredits", initialCredit)
			.hasFieldOrPropertyWithValue("receiveTimeout", receiveTimeout)
			.hasFieldOrPropertyWithValue("gracefulShutdownPeriod", gracefulShutdownPeriod);
	}

	@Test
	void autoConfigurationBacksOffWhenUserProvidesContainerFactory() {
		AmqpMessageListenerContainerFactory containerFactory = mock(AmqpMessageListenerContainerFactory.class);
		this.contextRunner
			.withBean(AmqpDefaultConfiguration.DEFAULT_AMQP_LISTENER_CONTAINER_FACTORY_BEAN_NAME,
					AmqpMessageListenerContainerFactory.class, () -> containerFactory)
			.run((context) -> {
				assertThat(context).hasSingleBean(AmqpMessageListenerContainerFactory.class)
					.doesNotHaveBean(MethodAmqpMessageListenerContainerFactory.class);
				assertThat(context.getBean(AmqpMessageListenerContainerFactory.class)).isEqualTo(containerFactory);
			});
	}
	...
	static class TestAmqpListenerEndpoint extends MethodAmqpListenerEndpoint {

		TestAmqpListenerEndpoint() {
			super(TestListener.class,
					Objects.requireNonNull(ReflectionUtils.findMethod(TestListener.class, "listen", String.class)));
		}

		@Override
		protected boolean isDefaultRequeueRejected() {
			return super.isDefaultRequeueRejected();
		}
